...
source_major_staged_weight_compensation="${CASA_RS_VLASS_SOURCE_MAJOR_STAGED_WEIGHT_COMPENSATION:-0}"
...
if [[ ( "$source_major_full_compensation" == "1" && "$source_major_weight_compensation" == "1" )
    || ( "$source_major_full_compensation" == "1" && "$source_major_staged_weight_compensation" == "1" )
    || ( "$source_major_weight_compensation" == "1" && "$source_major_staged_weight_compensation" == "1" ) ]]; then
...
if [[ "$source_major_staged_weight_compensation" == "1" ]]; then
    label="${label}-source-major-staged-weight-compensation"
    experimental_environment+=(
        CASA_RS_EXPERIMENTAL_AWPROJECT_SOURCE_MAJOR_STAGED_WEIGHT_COMPENSATION=1
    )
elif [[ "$source_major_staged_weight_compensation" != "0" ]]; then
    echo "CASA_RS_VLASS_SOURCE_MAJOR_STAGED_WEIGHT_COMPENSATION must be 0 or 1" >&2
    exit 2
fi

if [[ "$source_major_weight_compensation" == "1" ]]; then
    label="${label}-source-major-weight-compensation"
    experimental_environment+=(
        CASA_RS_EXPERIMENTAL_AWPROJECT_SOURCE_MAJOR_WEIGHT_COMPENSATION=1
    )
elif [[ "$source_major_weight_compensation" != "0" ]]; then
