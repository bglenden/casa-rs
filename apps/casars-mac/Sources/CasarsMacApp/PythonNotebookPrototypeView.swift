...
struct PythonNotebookPrototypeView: View {
    ...
    private var cellNavigator: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                Text("Python cells")
                    .workbenchFont(.headline)
                Spacer()
                Text("\(projection?.cells.count ?? 0)")
                    .workbenchFont(.caption, weight: .semibold)
                    .foregroundStyle(.secondary)
            }
            .padding(12)
                ...
                LazyVStack(spacing: 4) {
                    ForEach(projection?.cells ?? []) { cell in
                        Button {
                            ...
                            .padding(9)
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .background(cell.id == projection?.selectedCellID ? Color.accentColor.opacity(0.13) : Color.clear)
            ...
            Divider()
            Label(
                "\(projection?.insertedPlotCount ?? 0) plot revision(s) inserted",
                systemImage: "book.pages"
            )
            .workbenchFont(.caption)
            .foregroundStyle(.secondary)
    ...
    @ViewBuilder
    private var cellWorkspace: some View {
        if let cell = selectedCell {
            ...
            .background(Color(nsColor: .textBackgroundColor))
            .accessibilityIdentifier("pythonPrototype.cellWorkspace")
        } else {
            VStack(spacing: 10) {
                Image(systemName: "chevron.left.forwardslash.chevron.right")
                    .font(.system(size: 32))
                    .foregroundStyle(.secondary)
                Text("Select a Python cell")
                    .workbenchFont(.title3, weight: .semibold)
                Text("Choose a cell to review its code, outputs, and plot revisions.")
                    .foregroundStyle(.secondary)
            }
    ...
    private func cellHeader(_ cell: PrototypePythonCell) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 3) {
                Text(cell.title)
                    .workbenchFont(.title3, weight: .semibold)
                Text(cell.owner == .ai ? "AI-proposed code · exact-source approval required" : "User code · normal user authority")
                    .workbenchFont(.caption)
                    .foregroundStyle(cell.owner == .ai ? .purple : .secondary)
                    .accessibilityIdentifier("pythonPrototype.ownerDisclosure")
            }
            Spacer()
            Button {
                store.runPrototypePythonCell(cell.id)
