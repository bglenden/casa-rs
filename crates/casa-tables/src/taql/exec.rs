...
fn col_schema_from_value(name: &str, val: Option<&Value>) -> crate::schema::ColumnSchema {
    use casa_types::PrimitiveType as PT;
    match val {
        Some(Value::Scalar(s)) => {
            let pt = match s {
...
fn execute_alter_table(
    ...
    use casa_types::{PrimitiveType, ScalarValue, Value};
    ...
    match &alt.operation {
        ...
        AlterOperation::AddRow { count } => {
            ...
            for _ in 0..n {
                ...
                let fields: Vec<casa_types::RecordField> = table
                    .schema()
                    .map(|s| s.columns())
                    .unwrap_or(&[])
                    .iter()
                    .filter_map(|col: &crate::ColumnSchema| {
                        col.data_type().map(|pt| {
                            ...
                            casa_types::RecordField::new(col.name(), default_val)
                        })
                    })
                    .collect();
                table
                    .add_row(casa_types::RecordValue::new(fields))
                    .map_err(|e| TaqlError::Table(e.to_string()))?;
            }
...
fn parse_data_type(type_str: &str) -> Result<casa_types::PrimitiveType, TaqlError> {
...
fn default_value_for_type(ptype: casa_types::PrimitiveType) -> casa_types::Value {
    use casa_types::{PrimitiveType as PT, ScalarValue as SV, Value};
    match ptype {
...
/// Convert an `eval::ArrayValue` to a `casa_types::Value::Array`.
///
...
fn expr_array_to_value(arr: &super::eval::ArrayValue) -> Value {
    use casa_types::ArrayValue as AV;
