...
use casa_imaging_reconstruction::{
    ...
    runtime_adapter::{
        ...
        GRIDDED_NORMAL_LANE_COUNT, GRIDDED_NORMAL_PARTITION_COUNT, GriddedNormalCompilationPlan,
        ...
        GriddedNormalRoutingMeasurements, GriddedNormalStorageLayout, GriddedNormalStoragePlan,
        GriddedNormalWork, PreparedSpectralOperator, PreparedSpectralOperatorRecycle,
...
use crate::bounded_stream::{
    ...
    BoundedStreamPlan, KernelPartition, OrderedBlockSource, PartitionedKernel, WorkIdentity,
    execute_bounded,
};
use crate::{
        ...
        match self {
            ...
            Self::Topology {
                ...
                let useful_lanes = performance_cpu_cores.min(
                    u64::try_from(GRIDDED_NORMAL_LANE_COUNT)
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct GriddedNormalReplayBatchSchedule {
    maximum_frames: usize,
    frame_counts: Arc<[usize]>,
    source_slot_bytes: u64,
...
impl GriddedNormalReplayWindowPlan {
    fn for_program(
        ...
        let record_bytes = u64::try_from(program.record_bytes())
            .map_err(|_| CompleteDataPlanError::ResidencyOverflow)?;
        let prediction_width = program.prediction_width();
    ...
    pub(crate) fn plan_frame_payloads(
        ...
        if record_bytes == 0
            || prediction_width == 0
            || frames.iter().any(|&(payload_bytes, records)| {
                u64::try_from(record_bytes)
                    .ok()
                    .and_then(|record_bytes| records.checked_mul(record_bytes))
                    != Some(payload_bytes)
            })
        {
            return Err(CompleteDataPlanError::PlanMismatch);
        }
        let minimum_working_set = Self::minimum_working_set_bytes(frames, prediction_width)?;
        ...
        for (index, &(frame_payload_bytes, records)) in frames.iter().enumerate() {
            ...
            let complete_working_set = complete_source_capacity
                ...
                .ok_or(CompleteDataPlanError::ResidencyOverflow)?;
            if frame_count > 0 && complete_working_set > working_set_bytes {
        ...
        let planned_working_set = maximum_source_bytes
            ...
            .ok_or(CompleteDataPlanError::ResidencyOverflow)?;
        let batch_schedules = (1..=maximum_window_frames)
            .map(|maximum_frames| {
                ...
                Ok(GriddedNormalReplayBatchSchedule {
                    maximum_frames,
    ...
    fn batch_schedule(
        &self,
        maximum_frames: usize,
    ) -> io::Result<&GriddedNormalReplayBatchSchedule> {
        self.batch_schedules
            .iter()
            .find(|schedule| schedule.maximum_frames == maximum_frames)
            .ok_or_else(|| {
                io::Error::other("gridded-normal replay batch lacks a sealed window schedule")
            })
    }
}
...
impl GriddedNormalReplayCompilation {
    ...
    fn create(
        ...
        Ok(Self {
            ...
            writer: Some(
                ManagedSpillWriter::create(storage, admission.spill).map_err(io::Error::other)?,
            ),
            spill: None,
            #[cfg(test)]
            stage_timings: None,
...
impl FrozenGriddedNormalReplay {
    ...
    pub(crate) const fn latest_cache_load_measurements(&self) -> Option<ManagedSpillMeasurements> {
        self.latest_cache_load
    }

    pub(crate) const fn latest_prefetch_measurements(&self) -> Option<ManagedSpillMeasurements> {
        self.latest_prefetch
    }

    pub(crate) const fn latest_cache_resident_bytes(&self) -> Option<u64> {
        self.latest_cache_resident_bytes
    }

    pub(crate) fn prepare_retained_source(
        &mut self,
        context: WorkExecutionContext<'_>,
    ) -> io::Result<u64> {
        ...
        let batch_size = usize::try_from(context.knobs().batch_size)
            .map_err(|_| io::Error::other("gridded-normal replay batch overflow"))?;
        let schedule = self
            .window_plan
            .as_ref()
            .ok_or_else(|| io::Error::other("gridded-normal replay lacks its window plan"))?
            .batch_schedule(batch_size)?;
        let cache_capacity = context
            .node()
            .claims
            .iter()
            .find_map(|claim| {
                (claim.resource == LeaseResource::ResidentCache).then_some(claim.amount)
            })
            .ok_or_else(|| io::Error::other("retained replay cache claim missing"))?;
        let read_buffer = context
            .node()
            .claims
            .iter()
            .find_map(|claim| {
                (claim.resource == LeaseResource::IoBuffer(crate::IoBufferKind::SourceReadAhead))
        ...
        if cache_capacity < self.descriptor().bytes()
            ...
            || !context
                ...
                .claims
                .iter()
                .any(|claim| matches!(claim.resource, LeaseResource::StorageReadRate { .. }))
            || !context
                .node()
                .claims
                .iter()
            ...
            || !context
                ...
                .claims
                .iter()
                .any(|claim| claim.resource == LeaseResource::FileDescriptors)
        {
            return Err(io::Error::other(
                "retained replay load lacks its complete planned resources",
            ));
        }
        let source = self
            .spill
        ...
        if retained_bytes > cache_capacity
            || !context
                .allocations()
                .iter()
                .any(|allocation| allocation.capacity_bytes() >= retained_bytes)
        {
            return Err(io::Error::other(
                "retained replay exceeded its plan-owned backing allocation",
            ));
        }
    ...
    pub(crate) fn prepare_managed_source(
        &mut self,
        context: WorkExecutionContext<'_>,
    ) -> io::Result<()> {
        let is_prefetch_route =
            context.node().kind == WorkKind::Prefetch && context.knobs().prefetch;
        let is_direct_replay = context.node().kind == WorkKind::Spill && !context.knobs().prefetch;
        if (!is_prefetch_route && !is_direct_replay) || self.prepared_source.is_some() {
            return Err(io::Error::other(
                "gridded-normal managed backing was prepared at the wrong route node",
            ));
        }
        let batch_size = usize::try_from(context.knobs().batch_size)
            .map_err(|_| io::Error::other("gridded-normal replay batch overflow"))?;
        ...
        if !claims.iter().any(|claim| {
            claim.resource == LeaseResource::IoBuffer(crate::IoBufferKind::SpillRead)
                && claim.amount == minimum_buffer_bytes
        }) || !claims
            .iter()
            .any(|claim| matches!(claim.resource, LeaseResource::StorageReadRate { .. }))
            || !claims
    ...
    pub(crate) fn execute_bounded(
        ...
        if self.prepared_batch_size != Some(maximum_frames_per_block) {
            return Err(io::Error::other(
        ...
        let plan = BoundedStreamPlan::new::<GriddedNormalWork, GriddedNormalPartial>(
            ...
            batch_schedule
                ...
                .ok_or_else(|| io::Error::other("gridded-normal replay capacity overflow"))?,
...
impl PartitionedKernel<ManagedSpillWindowStorage> for GriddedNormalReplayKernel {
    ...
    fn partition_count(
        ...
        let records = storage.frames().try_fold(0_u64, |total, frame| {
            ...
            let records = u64::try_from(frame.payload().len() / self.record_bytes)
    ...
    fn partition(
        ...
        let partition = self
            .state
            .state
            .two_domain_window_partition(first_sequence, storage.frame_count(), local_ordinal)
            .map_err(CompleteDataOperatorError::Owner)?;
...
///
/// Reconstruction retains one slot per admitted frame ordinal, four complete-
/// group prediction lanes, and one stable center-tile route per record. The
/// predicted values and routing metadata are shared by all workers rather than
...
pub(crate) fn project_gridded_normal_compilation(
    ...
    let capacity =
        ManagedSpillBudget::for_bounded_stream(payload_capacity, frame_bytes, source_blocks)
            .map_err(io::Error::other)?;
    let spill = ManagedSpillBudget::new(capacity.maximum_artifact_bytes(), frame_bytes)
        .map_err(io::Error::other)?;
    let compiler = GriddedNormalCompilationPlan::new(
        problem,
        max_block_samples,
        frame_records,
        frame_records,
...
impl CompleteDataPlanFragment {
    ...
    pub(crate) fn channel_major_with_preparation_node(
        ...
        for depth in 1..=total_channels {
            ...
            let mut residency = project_residency(
                problem,
                &specification,
                workload,
        ...
        for core_start in (0..total_channels).step_by(admitted_depth) {
            let core_depth = admitted_depth.min(total_channels - core_start);
            let specification =
                SpectralOperatorSpecification::for_slab(problem, core_start, core_depth)?;
            let workload = spectral_operator_workload(
                &specification,
    ...
    pub(crate) fn slab_count(&self) -> usize {
        self.slab_specifications.len()
    }

    pub(crate) fn slab_specification(
        &self,
        ordinal: usize,
    ) -> Option<&SpectralOperatorSpecification> {
        self.slab_specifications.get(ordinal)
    }

    pub(crate) fn slab_workload(&self, ordinal: usize) -> Option<SpectralOperatorWorkload> {
        self.slab_workloads.get(ordinal).copied()
    }
    ...
    /// plan-listed low-memory route after the canonical prepared state has
    /// been discarded. The alternate node may change execution mechanics but
    /// is sealed to the same problem, workload, allocations, and numerics.
    pub(crate) fn recompute(
        ...
        if context.node().kind != WorkKind::Preparation
            || !context
                .node()
                .dependencies
                .contains(&WorkDependency::Work(self.preparation_node.clone()))
        {
            return Err(CompleteDataPlanError::WrongExecutionNode);
        }
        self.prepare_operator_state(context)
    ...
    pub(crate) fn reprepare_slab(
        &self,
        context: WorkExecutionContext<'_>,
        ordinal: usize,
        recycle: PreparedSpectralOperatorRecycle,
    ) -> Result<CompleteDataPreparedState, CompleteDataPlanError> {
        if context.node().id != self.replay_node || ordinal == 0 {
            return Err(CompleteDataPlanError::WrongExecutionNode);
        }
        let specification = self
            .slab_specification(ordinal)
            .cloned()
            .ok_or(CompleteDataPlanError::PlanMismatch)?;
        let workload = self
...
fn project_residency(
    ...
    let grid_bytes = match execution_role {
        CompleteDataExecutionRole::SelectedObservation => workload
            .grid_complex_values()
        ...
        CompleteDataExecutionRole::GriddedArtifact => {
            ...
            residency
                .peak_complex_values()
                .checked_mul(complex_bytes)
                .and_then(|bytes| bytes.checked_add(residency.metadata_bytes()))
                .ok_or(CompleteDataPlanError::ResidencyOverflow)?
        }
    };
...
mod tests {
    ...
    #[test]
    fn t42_replay_window_plan_charges_dynamic_record_and_prediction_widths() {
        let record_bytes = 48;
        let prediction_width = 3;
        let frames = [(48, 1), (96, 2)];
        let minimum =
            GriddedNormalReplayWindowPlan::minimum_working_set_bytes(&frames, prediction_width)
                .expect("dynamic singleton minimum");
        let plan = GriddedNormalReplayWindowPlan::for_frame_payloads(
            &frames,
            minimum,
            record_bytes,
        ...
        let route_capacity =
            gridded_normal_route_capacity_bytes(2, 1, prediction_width).expect("route capacity");

        assert_eq!(plan.frame_counts(), &[1, 1]);
        assert_eq!(
            plan.source_slot_bytes(),
            96 + super::FRAME_HEADER_BYTES as u64
        );
        assert_eq!(plan.route_capacity_bytes(), route_capacity);
        assert_eq!(
            plan.working_set_bytes(),
            exact_working_set(96 + super::FRAME_HEADER_BYTES as u64, route_capacity)
        );
    ...
    #[test]
    fn low_memory_batch_repackages_planned_replay_windows() {
        let frames = [(32, 1), (32, 1), (32, 1), (32, 1), (3_200, 100)];
        let route_capacity = gridded_normal_route_capacity_bytes(103, 4, TEST_PREDICTION_WIDTH)
            .expect("route capacity");
        let working_set = exact_working_set(3_272, route_capacity);
        ...
        assert_eq!(plan.frame_counts(), &[4, 1]);
        assert_eq!(
            plan.batch_schedule(2)
                .expect("adapted batches")
                .frame_counts
                .as_ref(),
            &[2, 2, 1]
        );
        assert_eq!(
            plan.batch_schedule(1)
                .expect("minimal batches")
                .frame_counts
                .as_ref(),
            &[1, 1, 1, 1, 1]
        );
    ...
    fn replay_window_budget_uses_exact_data_and_topology_quantum() {
        ...
        let expected_minimum = exact_working_set(
            65_536 + super::FRAME_HEADER_BYTES as u64,
            gridded_normal_route_capacity_bytes(2_048, 1, TEST_PREDICTION_WIDTH)
                .expect("route capacity"),
        );
        let expected_lane_budget = expected_minimum * 4;
        assert_eq!(minimum, expected_minimum);
        assert_eq!(
            four_lane.working_set_bytes(minimum).unwrap(),
            expected_lane_budget
        );
        assert_eq!(
            sector_capped.working_set_bytes(minimum).unwrap(),
            expected_lane_budget
        );
    }
    ...
    fn replay_window_binding_reauthorizes_only_the_identical_plan() {
        let frames = [(32, 1), (96, 3), (32, 1)];
        let first = GriddedNormalReplayWindowPlan::for_frame_payloads(
            &frames,
            1_030,
            TEST_RECORD_BYTES,
            TEST_PREDICTION_WIDTH,
        ...
        .expect("singleton minimum");
        let changed = GriddedNormalReplayWindowPlan::for_frame_payloads(
            &frames,
            singleton_minimum,
            TEST_RECORD_BYTES,
            TEST_PREDICTION_WIDTH,
