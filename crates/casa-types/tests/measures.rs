...
fn epoch_from_quantity() {
    let q = casa_types::quanta::Quantity::new(J2000_MJD, "d").unwrap();
...
fn epoch_from_quantity_wrong_units() {
    let q = casa_types::quanta::Quantity::new(1.0, "m").unwrap();
...
fn radial_velocity_record_roundtrip() {
    use casa_types::measures::radial_velocity::{MRadialVelocity, RadialVelocityRef};
    let rv = MRadialVelocity::new(100_000.0, RadialVelocityRef::LSRK);
    let rec = casa_types::measures::radial_velocity_to_record(&rv);
    let decoded = casa_types::measures::radial_velocity_from_record(&rec).unwrap();
