...
Verification: `python3 -m unittest tools/perf/imager/test_run_workload.py tools/perf/imager/test_stage_wave1_datasets.py`; `bash -n scripts/bench-imager-vs-casa.sh`; `tools/perf/imager/stage_wave1_datasets.py --data-root /Volumes/GLENDENNING/casa-rs-imperformance --materialize-workloads --output-dir target/imperformance-wave1/issue251-plan`; selected `tools/perf/imager/run_workload.py` runs listed below
...
| `mtmfs-wideband-sentinel` | `wave1-alma-single-small` | MFS, standard, MT-MFS, `512x512`, 8 channels, Briggs, `nterms=2`, `niter=25` | `.image.tt0`, `.residual.tt0`, `.psf.tt0` | `target/imperformance-wave1/issue251-final-runs/20260518T144642Z-wave1-alma-single-small-mtmfs-wideband-sentinel-7abcb6a7.json` | n/a | n/a | n/a | BLOCKED: Rust exits before timing with `MTMFS currently supports gridder='standard' only` even though the manifest requests `gridder='standard'` | n/a |
...
- The MT-MFS sentinel is blocked before timing. The manifest now requests a
  real MT-MFS shape (`deconvolver='mtmfs'`, `nterms=2`, Taylor-term product
  suffixes), but the current Rust frontend rejects the generated single-field
  dataset before producing timings.
...
| MT-MFS sentinel | Blocked before timing | frontend/gridder-mode routing for generated single-field MT-MFS | Fix MT-MFS benchmark routing or choose a supported standard-gridder MT-MFS dataset before using this sentinel for performance conclusions. |
...
4. Repair MT-MFS sentinel routing before making wideband performance claims.
