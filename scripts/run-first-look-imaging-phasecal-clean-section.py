#!/usr/bin/env python3
"""Run the ALMA First Look Imaging cleaned phase-calibrator tutorial section."""
...
def write_docs(pack_root: Path, evidence: dict[str, Any]) -> None:
    ...
    comparison_png = ".casa-rs/evidence/05-phase-cal-clean-comparison.png"
    gui_run_png = ".casa-rs/screenshots/gui/05-phase-cal-clean-gui-run.png"
    gui_parameters_png = ".casa-rs/screenshots/gui/05-phase-cal-clean-gui-parameters.png"
    gui_imexplore_png = ".casa-rs/screenshots/gui/05-phase-cal-clean-gui-imexplore.png"
    tui_png = ".casa-rs/screenshots/headless/05-phase-cal-clean-tui.png"
    cli_image_png = ".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.cli.image.png"
    cli_residual_png = ".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.cli.residual.png"
    python_image_png = ".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.python.image.png"
    tui_image_png = ".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.tui.image.png"
    gui_image_png = ".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.gui.image.png"
    ...
    markdown = f"""# 05 - Clean the phase calibrator

CASA tutorial chunk: after inspecting the dirty phase-calibrator image, the CASA
guide uses an interactive mask around the central point source and continues
CLEAN/deconvolution. For regression execution this section uses the CASA
Guide's non-interactive equivalent: the same phase calibrator image parameters,
`niter=10000`, `threshold="15mJy"`, and a rectangular clean mask around the
source.
...
| --- | --- | --- |
| CASA | `tclean` | `vis="twhya_calibrated.ms"`, `imagename=".casa-rs/workspace/oracle/05-phase-cal-clean/phase_cal_clean.casa"`, `field="3"`, auto data-column selection, `specmode="mfs"`, `deconvolver="hogbom"`, `gridder="standard"`, `imsize=[250,250]`, `cell=["0.1arcsec"]`, `weighting="briggs"`, `robust=0.5`, `niter=10000`, `threshold="15mJy"`, `interactive=False`, `mask="box[[100pix,100pix],[150pix,150pix]]"` |
| CLI | `casars-imager` | `--ms twhya_calibrated.ms --imagename .casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.cli --imsize 250 --cell-arcsec 0.1 --field 3 --phasecenter-field 3 --specmode mfs --deconvolver hogbom --weighting briggs --robust 0.5 --threshold-jy 0.015 --niter 10000 --mask-box 100,100,150,150 --pblimit 0.2 --write-pb` |
| Python | `casars.tasks.imager.mfs` | `measurement_set="twhya_calibrated.ms"`, `image_name=".casa-rs/workspace/native/05-phase-cal-clean/phase_cal_clean.python"`, `image_size=250`, `cell_arcsec=0.1`, `field_ids=[3]`, `phasecenter_field=3`, `weighting="briggs"`, `robust=0.5`, `deconvolver="hogbom"`, `niter=10000`, `threshold_jy=0.015`, `mosaic_pb_limit=0.2`, `write_pb=True`, `dirty_only=False`, `mask_boxes=[(100,100,150,150)]` |
...
The cleaned phase-calibrator image should retain the compact central source but
...
<head>
<meta charset="utf-8">
<title>ALMA First Look Imaging - 05 Clean phase calibrator</title>
<style>
...
<body>
<h1>ALMA First Look Imaging: 05 Clean phase calibrator</h1>
<p>The CASA guide moves from the dirty phase-calibrator image to a mask-driven CLEAN. This evidence run uses the non-interactive CASA equivalent with <code>niter=10000</code>, <code>threshold="15mJy"</code>, and <code>mask="box[[100pix,100pix],[150pix,150pix]]"</code>.</p>
<p>CASA source page: <a href="{CASA_GUIDE_URL}">{CASA_GUIDE_URL}</a></p>
<h2>Parameters</h2>
<table>
<tr><th>Surface</th><th>Task</th><th>Parameters</th></tr>
<tr><td>CASA</td><td><code>tclean</code></td><td><code>field=3</code>, auto data-column selection, <code>imsize=[250,250]</code>, <code>cell=["0.1arcsec"]</code>, <code>weighting=briggs</code>, <code>robust=0.5</code>, <code>niter=10000</code>, <code>threshold=15mJy</code>, <code>mask=box[[100pix,100pix],[150pix,150pix]]</code></td></tr>
<tr><td>CLI</td><td><code>casars-imager</code></td><td><code>--field 3 --phasecenter-field 3 --imsize 250 --cell-arcsec 0.1 --weighting briggs --robust 0.5 --niter 10000 --threshold-jy 0.015 --mask-box 100,100,150,150 --pblimit 0.2 --write-pb</code></td></tr>
<tr><td>Python</td><td><code>casars.tasks.imager.mfs</code></td><td><code>field_ids=[3]</code>, <code>phasecenter_field=3</code>, <code>niter=10000</code>, <code>threshold_jy=0.015</code>, <code>mosaic_pb_limit=0.2</code>, <code>write_pb=True</code>, <code>mask_boxes=[(100,100,150,150)]</code></td></tr>
...
<p>The cleaned image should show the same compact phase calibrator with reduced sidelobe structure relative to the dirty image. The model should be non-zero inside the mask, and the residual should be lower than the dirty residual.</p>
<h2>Evidence</h2>
<div class="grid">
...
def run_section(args: argparse.Namespace) -> dict[str, Any]:
    ...
    casa_prefix = oracle_dir / "phase_cal_clean.casa"
    cli_prefix = native_dir / "phase_cal_clean.cli"
    python_prefix = native_dir / "phase_cal_clean.python"
    tui_prefix = native_dir / "phase_cal_clean.tui"
    gui_prefix = native_dir / "phase_cal_clean.gui"
    ...
    tui_capture_run = run_command(
        [
            ...
            str(pack_root),
            "--output",
            str(headless_dir / "05-phase-cal-clean-tui.png"),
            "--width",
            "2400",
            "--height",
            "1600",
            ...
            "12",
            "--settle-seconds",
            str(min(max(240, args.timeout_seconds), 300)),
            "--input-event",
            "500:r",
            "--input-event",
            "1500:r",
    ...
    gui_run = run_command(
        [
            ...
            "false",
            "--output",
            str(gui_dir / "05-phase-cal-clean-gui-run.png"),
            "--width",
            "1800",
            "--height",
            "1200",
    ...
    comparison_run = run_command(
        [
            ...
            comparison_code(
                ...
                str(evidence_dir / "05-phase-cal-clean-comparison.png"),
            ),
        ],
        cwd=pack_root,
        env=env,
    ...
    gui_parameters_run = run_command(
        [
            ...
            "false",
            "--output",
            str(gui_dir / "05-phase-cal-clean-gui-parameters.png"),
            "--width",
            "1800",
            "--height",
            "1200",
    ...
    gui_imexplore_run = run_command(
        [
            ...
            "--output",
            str(gui_dir / "05-phase-cal-clean-gui-imexplore.png"),
            "--width",
            "1800",
            "--height",
            "1200",
    ...
    evidence = {
        ...
        "tui": {
            "run": tui_capture_run,
            "screenshot": str((headless_dir / "05-phase-cal-clean-tui.png").relative_to(pack_root)),
            "products": product_records(tui_prefix, pack_root),
        },
        "gui": {
            "run_screenshot": str((gui_dir / "05-phase-cal-clean-gui-run.png").relative_to(pack_root)),
            "parameters_screenshot": str((gui_dir / "05-phase-cal-clean-gui-parameters.png").relative_to(pack_root)),
            "imexplore_screenshot": str((gui_dir / "05-phase-cal-clean-gui-imexplore.png").relative_to(pack_root)),
