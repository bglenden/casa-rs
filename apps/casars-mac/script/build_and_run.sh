...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --show-prototype)
      if [[ $# -lt 2 ]]; then
        echo "--show-prototype requires: notebook, python, tutorial, ai, or onboarding" >&2
        exit 2
      fi
      case "$2" in
        notebook|python|tutorial|ai|onboarding)
          SHOW_PROTOTYPE="$2"
          ;;
        *)
          echo "--show-prototype requires: notebook, python, tutorial, ai, or onboarding" >&2
    ...
    --prototype-state)
      ...
      case "$2" in
        happy-path|external-conflict|failure|nonresponsive|rate-limited|checksum-failure|disk-failure|offline|unsafe-archive)
          ...
          ;;
        *)
          echo "--prototype-state requires a state supported by the selected prototype" >&2
          exit 2
          ;;
      esac
      shift 2
    ...
    --imagename|--output-prefix|--image-size|--imsize|--image-width|--image-height|--cell-arcsec|--spectral-mode|--specmode|--channel-start|--channel-count|--niter|--threshold-jy|--dirty-only)
      ...
      ;;
    *)
      echo "usage: $0 [run|--debug|--logs|--verify|--stage-only] [--project PATH|--imager-ms PATH|--tutorial-pack PATH [--tutorial-section ID]|--show-prototype notebook|python|tutorial|ai|onboarding [--prototype-state STATE]|--empty] [--show-imager-progress-mockup] [--run-active-task] [imager launch overrides]" >&2
      exit 2
      ;;
  esac
done
...
case "$SHOW_PROTOTYPE:$PROTOTYPE_STATE" in
  :happy-path|notebook:happy-path|notebook:external-conflict|python:happy-path|python:failure|python:nonresponsive|tutorial:happy-path|tutorial:checksum-failure|tutorial:disk-failure|tutorial:offline|tutorial:unsafe-archive|ai:happy-path|ai:rate-limited|ai:nonresponsive|onboarding:happy-path)
    ;;
  *)
...
case "$MODE" in
  ...
  --stage-only|stage)
    ...
    ;;
  *)
    echo "usage: $0 [run|--debug|--logs|--verify|--stage-only] [--project PATH|--imager-ms PATH|--tutorial-pack PATH|--show-prototype notebook|python|tutorial|ai|onboarding [--prototype-state STATE]|--empty] [--show-imager-progress-mockup] [--run-active-task] [imager launch overrides]" >&2
    exit 2
    ;;
esac
