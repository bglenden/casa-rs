...
usage() {
  cat <<'EOF'
Usage:
  scripts/render-msexplore-stacked-side-by-side.sh \
    --ms /path/to.ms \
...
spw="0"
scan="1"
rust_label="casa-rs amplitude / phase vs time (stacked)"
casa_label="CASA plotms amplitude / phase vs time (stacked)"
...
echo "Rendering CASA stacked plot..."
(cd "$tmpdir" && \
CASA_VIS="$ms_path" \
CASA_OUT="$casa_png" \
CASA_FIELD="$field" \
