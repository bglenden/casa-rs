...
fn run_session() -> Result<(), String> {
    ...
    for line_result in stdin.lock().lines() {
        ...
        let request: BrowserRequestEnvelope = match serde_json::from_str(&line) {
            Ok(request) => request,
            Err(error) => {
                let response = casars_tablebrowser_protocol::BrowserResponseEnvelope::error(
                    "invalid_json",
        ...
        let response = if request.version != PROTOCOL_VERSION {
            casars_tablebrowser_protocol::BrowserResponseEnvelope::error(
                "unsupported_version",
                format!(
            ...
            match (&mut browser, &request.command) {
                ...
                (None, BrowserCommand::OpenRoot { path, viewport }) => {
                    match TableBrowser::open(path) {
                        ...
                        Err(error) => casars_tablebrowser_protocol::BrowserResponseEnvelope::error(
                            "open_root_failed",
                            error.to_string(),
                        ),
                    }
                }
                (None, _) => casars_tablebrowser_protocol::BrowserResponseEnvelope::error(
                    "session_not_open",
