...
literal_v1_receipt_sha="1dbed734c0f9dc3966038302c673c2ddeae7c812e4507408b4c56647db3d3c2c"
literal_v1_evidence_sha="dfb6f809c9e96b6007321b293ee147b961a39bfc65968c21637f65058afc8d38"
...
fail() {
    echo "VLASS casa-rs native-geometry audit: $*" >&2
    exit 1
}
...
[[ "$(sha256 "${literal_v1_receipt}")" == "${literal_v1_receipt_sha}" ]] ||
...
mkdir "${case_dir}"
{
    ...
    printf 'literal_v1_receipt_sha256\t%s\n' "${literal_v1_receipt_sha}"
    printf 'literal_v1_evidence_sha256\t%s\n' "${literal_v1_evidence_sha}"
...
[[ "${comparison_status}" == "0" ]] ||
    fail "native-geometry evidence was invalid; preserve ${case_dir}"
[[ -s "${comparison}" ]] ||
...
require_clean_checkout
require_exact_checkpoint
printf 'result\tvalidly-classified-native-geometry-evidence\n' >>"${provenance}"
echo "VLASS casa-rs native-geometry evidence classified as ${classification}: ${comparison}"
