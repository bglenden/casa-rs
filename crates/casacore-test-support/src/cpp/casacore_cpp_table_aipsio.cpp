...
void write_aipsio_unsigned_arrays_impl(const std::string& path) {
    casacore::TableDesc td("", casacore::TableDesc::Scratch);
...
void verify_aipsio_unsigned_arrays_impl(const std::string& path) {
    casacore::Table table(path, casacore::Table::Old);
...
void write_aipsio_string_array_impl(const std::string& path) {
    casacore::TableDesc td("", casacore::TableDesc::Scratch);
...
void verify_aipsio_string_array_impl(const std::string& path) {
    casacore::Table table(path, casacore::Table::Old);
    if (table.nrow() != 3) throw std::runtime_error("expected 3 rows");
...
void write_aipsio_complex64_2d_array_impl(const std::string& path) {
    casacore::TableDesc td("", casacore::TableDesc::Scratch);
...
void verify_aipsio_complex64_2d_array_impl(const std::string& path) {
    casacore::Table table(path, casacore::Table::Old);
    if (table.nrow() != 3) throw std::runtime_error("expected 3 rows");
...
void write_aipsio_empty_table_impl(const std::string& path) {
    casacore::TableDesc td("", casacore::TableDesc::Scratch);
    td.addColumn(casacore::ScalarColumnDesc<casacore::Int>("col_i32"));
...
void verify_aipsio_empty_table_impl(const std::string& path) {
    casacore::Table table(path, casacore::Table::Old);
...
int32_t cpp_table_write_aipsio_unsigned_arrays(const char* path, char** out_error) {
    try { write_aipsio_unsigned_arrays_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_verify_aipsio_unsigned_arrays(const char* path, char** out_error) {
    try { verify_aipsio_unsigned_arrays_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_write_aipsio_string_array(const char* path, char** out_error) {
    try { write_aipsio_string_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_verify_aipsio_string_array(const char* path, char** out_error) {
    try { verify_aipsio_string_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_write_aipsio_complex64_2d_array(const char* path, char** out_error) {
    try { write_aipsio_complex64_2d_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_verify_aipsio_complex64_2d_array(const char* path, char** out_error) {
    try { verify_aipsio_complex64_2d_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_write_aipsio_empty_table(const char* path, char** out_error) {
    try { write_aipsio_empty_table_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_verify_aipsio_empty_table(const char* path, char** out_error) {
    try { verify_aipsio_empty_table_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
