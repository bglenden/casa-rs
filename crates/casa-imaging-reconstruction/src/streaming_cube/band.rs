...
use crate::{
    ModelGeneration, PolarizationOperator, SpectralOperatorError, SpectralOperatorPrimitives,
...
impl BandPlan {
    ...
    /// Conservative fresh-normal bound used before loading a prior normal
    /// window. Runtime adds that window's full read residency separately.
    pub fn full_refresh(&self) -> Self {
        Self {
...
impl<'a> EpochBand<'a> {
    fn prepare(
        ...
        generation: &'a ModelGeneration,
        mut prior: Option<SpectralOperatorPrimitives>,
        native_range: Range<usize>,
    ) -> Result<Self, SpectralOperatorError> {
        ...
        match (workspace.phase, prior.as_mut()) {
            (BandPhase::InitialZero, None)
                if generation.origin() == crate::ModelGenerationOrigin::Empty => {}
            (BandPhase::Full, None) => {}
            (BandPhase::Residual, Some(prior)) => prior.prepare_cube_reuse(
                workspace.geometry.image_shape,
    ...
    pub fn complete(
        ...
        Ok((
            SpectralOperatorPrimitives::from_cube_band(images, channels, model, self.prior)?,
            fft,
        ))
    }
}
