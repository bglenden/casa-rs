...
pub(super) fn save_native_region_record(
    ...
    match pixel_type {
        ...
        ImagePixelType::Complex32 => {
            let mut image = PagedImage::<casacore_types::Complex32>::open(path)?;
            image.put_region_record(name, record)?;
            image.save()
        }
        ImagePixelType::Complex64 => {
            let mut image = PagedImage::<casacore_types::Complex64>::open(path)?;
            image.put_region_record(name, record)?;
            image.save()
        }
    }
...
pub(super) fn rename_native_region_record_name(
    ...
    renamed.upsert(
        "comment",
        casacore_types::Value::Scalar(casacore_types::ScalarValue::String(new_name.to_string())),
    );
    if let Some(casacore_types::Value::Record(regions)) = renamed.get_mut("regions") {
            ...
            .collect::<Vec<_>>()
        {
            ...
            if let Some(casacore_types::Value::Record(child)) = regions.get_mut(&child_name) {
                child.upsert(
                    "comment",
                    casacore_types::Value::Scalar(casacore_types::ScalarValue::String(
                        new_name.to_string(),
                    )),
                );
            }
