...
impl LinearCoordinate {
    pub(crate) fn to_record(&self) -> RecordValue {
        ...
        rec.upsert(
            "crval",
            Value::Array(casa_types::ArrayValue::from_f64_vec(self.crval.clone())),
        );
        rec.upsert(
            "cdelt",
            Value::Array(casa_types::ArrayValue::from_f64_vec(self.cdelt.clone())),
        );
        rec.upsert(
            "crpix",
            Value::Array(casa_types::ArrayValue::from_f64_vec(self.crpix.clone())),
        );
        rec.upsert(
            "pc",
        ...
        rec.upsert(
            "axes",
            Value::Array(casa_types::ArrayValue::from_string_vec(self.names.clone())),
        );
        rec.upsert(
            "units",
            Value::Array(casa_types::ArrayValue::from_string_vec(self.units.clone())),
        );
