...
description: Use to implement an approved wave on a branch. Keeps work inside the approved boundary, runs preflight only when needed, commits and pushes the wave branch, and creates or updates the PR.
---
...
Implement a wave without silently widening the approved scope, then produce a review-ready PR.
...
1. Confirm the scope, non-goals, acceptance checks, and stop conditions.
...
10. Update issue closeout and PR material so they match the actual implementation and verification result.
11. Commit the finished wave changes on the wave branch.
12. Push the wave branch and create or update the PR.
13. Move the wave to `Review` only after the PR exists and carries the latest `verify` result.
