...
fn apply_complex_scale_offset(
    ...
    // Extract real and imaginary slices from the stored array.
    // Stored layout: [2, ...] in Fortran order. The first axis alternates re/im.
    let flat = stored_arr.as_slice().ok_or_else(|| {
        StorageError::FormatMismatch("ScaledComplexData: stored array not contiguous".to_string())
    })?;
