// StandardStMan fixture: ssm_variable_array.
...
void write_ssm_variable_array_impl(const std::string& path) {
    casacore::TableDesc td("", casacore::TableDesc::Scratch);
    // ndim=2, no shape argument and no FixedShape flag => variable-shape column
    ...
    // Row 0: shape [2,3], values 1.0 .. 6.0 (column-major: axis 0 varies fastest)
    {
...
void verify_ssm_variable_array_impl(const std::string& path) {
    ...
    // Row 0: shape [2,3], values 1.0 .. 6.0
    {
        ...
        if (arr(casacore::IPosition(2, 1, 0)) != 2.0f)  throw std::runtime_error("row 0 [1,0] mismatch");
        if (arr(casacore::IPosition(2, 0, 1)) != 3.0f)  throw std::runtime_error("row 0 [0,1] mismatch");
        if (arr(casacore::IPosition(2, 1, 1)) != 4.0f)  throw std::runtime_error("row 0 [1,1] mismatch");
        if (arr(casacore::IPosition(2, 0, 2)) != 5.0f)  throw std::runtime_error("row 0 [0,2] mismatch");
        if (arr(casacore::IPosition(2, 1, 2)) != 6.0f)  throw std::runtime_error("row 0 [1,2] mismatch");
    }
    // Row 1: shape [3,2], values 7.0 .. 12.0
    {
        ...
        if (arr(casacore::IPosition(2, 0, 0)) != 7.0f)  throw std::runtime_error("row 1 [0,0] mismatch");
        if (arr(casacore::IPosition(2, 1, 0)) != 8.0f)  throw std::runtime_error("row 1 [1,0] mismatch");
        if (arr(casacore::IPosition(2, 2, 0)) != 9.0f)  throw std::runtime_error("row 1 [2,0] mismatch");
        if (arr(casacore::IPosition(2, 0, 1)) != 10.0f) throw std::runtime_error("row 1 [0,1] mismatch");
        if (arr(casacore::IPosition(2, 1, 1)) != 11.0f) throw std::runtime_error("row 1 [1,1] mismatch");
        if (arr(casacore::IPosition(2, 2, 1)) != 12.0f) throw std::runtime_error("row 1 [2,1] mismatch");
    }
    // Row 2: shape [3,2], values 13.0 .. 18.0
    {
        casacore::IPosition expectedShape(2, 3, 2);
        ...
        if (arr(casacore::IPosition(2, 0, 0)) != 13.0f) throw std::runtime_error("row 2 [0,0] mismatch");
        if (arr(casacore::IPosition(2, 1, 0)) != 14.0f) throw std::runtime_error("row 2 [1,0] mismatch");
        if (arr(casacore::IPosition(2, 2, 0)) != 15.0f) throw std::runtime_error("row 2 [2,0] mismatch");
        if (arr(casacore::IPosition(2, 0, 1)) != 16.0f) throw std::runtime_error("row 2 [0,1] mismatch");
        if (arr(casacore::IPosition(2, 1, 1)) != 17.0f) throw std::runtime_error("row 2 [1,1] mismatch");
        if (arr(casacore::IPosition(2, 2, 1)) != 18.0f) throw std::runtime_error("row 2 [2,1] mismatch");
    }
    // Row 3: shape [2,3], values 19.0 .. 24.0
    {
        casacore::IPosition expectedShape(2, 2, 3);
        ...
        if (arr(casacore::IPosition(2, 0, 0)) != 19.0f) throw std::runtime_error("row 3 [0,0] mismatch");
        if (arr(casacore::IPosition(2, 1, 0)) != 20.0f) throw std::runtime_error("row 3 [1,0] mismatch");
        if (arr(casacore::IPosition(2, 0, 1)) != 21.0f) throw std::runtime_error("row 3 [0,1] mismatch");
        if (arr(casacore::IPosition(2, 1, 1)) != 22.0f) throw std::runtime_error("row 3 [1,1] mismatch");
        if (arr(casacore::IPosition(2, 0, 2)) != 23.0f) throw std::runtime_error("row 3 [0,2] mismatch");
        if (arr(casacore::IPosition(2, 1, 2)) != 24.0f) throw std::runtime_error("row 3 [1,2] mismatch");
    }
}
...
int32_t cpp_table_write_ssm_variable_array(const char* path, char** out_error) {
    try { write_ssm_variable_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
int32_t cpp_table_verify_ssm_variable_array(const char* path, char** out_error) {
    try { verify_ssm_variable_array_impl(path); return 0;
    } catch (const std::exception& e) { *out_error = make_error(e.what()); return -1;
    } catch (...) { *out_error = make_error("unknown exception"); return -1; }
}
