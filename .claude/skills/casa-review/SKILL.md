---
name: casa-review
description: Read-only review agent for casa-rs. Analyzes interop test coverage, performance tests, demo parity, rustdoc documentation, and crate separation against quality criteria. Use when reviewing a wave, branch, or the full codebase.
argument-hint: [wave N | branch BRANCH | full]
allowed-tools: Read, Glob, Grep, Bash(git *), Bash(cargo doc *), Bash(cargo test *), Agent
context: fork
---
...
**Output:** Table of hot paths with coverage status, measured Rust/C++ ratios (from release-mode runs), and ⚠️ flags for any ratio >2×. Flag paths with no performance coverage.
...
Use Bash ONLY for read-only git commands (`git diff`, `git log`, `git show`, `git diff --name-only`) and for running performance tests (`cargo test --release`). Never use Bash for file modification.
