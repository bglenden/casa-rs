...
| `alma/automasking` | Automasking Guide | 6.6.6 | <https://casaguides.nrao.edu/index.php/Automasking_Guide> | `listobs`, `tclean` with `usemask='auto-multithresh'`, `noisethreshold`, `sidelobethreshold`, `lownoisethreshold`, `minbeamfrac`, `negativethreshold`, `fastnoise` | `twhya_selfcal.ms.contsub.tgz`; expected products include `twhya_dirtycube.{image,residual,psf,sumwt}`, automasked cube image/residual/mask products such as `twhya_base_params.{image,residual,mask}`, and parameter-variation image/residual/mask products | #167 |
