...
impl Table {
    ...
    pub fn open_with_lock(
        ...
        match lock_opts.mode {
            ...
            LockMode::AutoLocking | LockMode::DefaultLocking => {
                ...
                if !acquired {
                    return Err(TableError::LockFailed {
                        path: options.path.display().to_string(),
                        message: "could not acquire initial read lock".to_string(),
                    });
                }
            }
        ...
        // Load the descriptor only after acquiring the requested initial lock.
        // `Table::open` preserves lazy row payloads, so selected-column reads
        // remain bounded while the retained lock protects this table view.
