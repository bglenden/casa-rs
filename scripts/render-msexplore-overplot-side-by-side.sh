...
usage() {
  cat <<'EOF'
Usage:
  scripts/render-msexplore-overplot-side-by-side.sh \
    --ms /path/to.ms \
...
spw="0"
scan="1"
rust_label="casa-rs amplitude vector/scalar overplot"
casa_label="CASA plotms amplitude vector/scalar overplot"
...
echo "Rendering CASA overplot page..."
(cd "$tmpdir" && \
CASA_VIS="$ms_path" \
CASA_OUT="$casa_png" \
CASA_FIELD="$field" \
