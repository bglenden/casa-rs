...
| Row/channel run payloads instead of scalar inbox items | Historical tests found row/channel runs removed scalar synchronization; scalar direct inbox took 258.34 s wall, 1020.53 s system, and 14.4 M drains (`imperformance-wave-2-experiment-summary.md:138-143`, `155-156`). CASA and LibRA likewise preserve row/channel/correlation buffer shape through their visibility-buffer and resampler seams. | The realistic 64-channel serial profile proves the current callback expands 5.15 GB of logical source data into 230.58 GB of 440-byte scalar handoffs per pass. Source reads take about 10.3-10.7 s, while consumers take 102.1-248.6 s; copying, hashing, repeated stencil construction, and scalar record assembly are leading leaves. | `casa-ms` owns physical block views; reconstruction owns scientific run consumption; runtime transports leases and accounts exact residency. | **Adapt in Delivery 1.** Replace the scalar hot handoff with one borrowed run-shaped payload on the existing bounded executor; preserve canonical traversal, deterministic reduction, block-boundary evidence, and the same `workers=1` path. |
