...
int32_t cpp_table_write_ism_scalar_primitives(const char* path, char** out_error) {
    try {
        ...
        *out_error = make_error("unknown exception in write_ism_scalar_primitives");
        return -1;
    }
}
...
int32_t cpp_table_verify_ism_scalar_primitives(const char* path, char** out_error) {
    try {
        ...
        *out_error = make_error("unknown exception in verify_ism_scalar_primitives");
        return -1;
    }
}

// ===== ISM slowly_changing fixture =====
// 10 rows where values repeat across consecutive rows, testing delta compression.
...
int32_t cpp_table_write_ism_slowly_changing(const char* path, char** out_error) {
    try {
        ...
        *out_error = make_error("unknown exception in write_ism_slowly_changing");
        return -1;
    }
}
...
int32_t cpp_table_verify_ism_slowly_changing(const char* path, char** out_error) {
    try {
        ...
        if (table.nrow() != 10) {
            throw std::runtime_error("expected 10 rows, got " +
                                     std::to_string(table.nrow()));
        }
        ...
        *out_error = make_error("unknown exception in verify_ism_slowly_changing");
        return -1;
    }
}
