...
| Visibility source traversal | overlap source reads with row preparation without changing sample order or residency bounds | shared `stream_imaging_source_read_ahead_blocks` producer/consumer path | standard MFS, mosaic MFS/MT-MFS replay, cube/cubedata, and mosaic cube share the path; the current maximum is two live blocks (one producer plus one consumer), with a zero-capacity rendezvous queue and slab guards when read-ahead would reduce residency/locality |
| Dirty-product FFT and normalization | transform and normalize PSF/residual/weight grids without changing product semantics | CPU finishers plus guarded Apple MPSGraph resident standard/mosaic finishers | eligible f32 products may remain device-resident through correction, normalization, and peak reduction; Auto keeps small or unsupported work on CPU and command failures fall back to the shared CPU finisher; f64 remains CPU |
...
- Apple GPU product finishing is an execution choice, not a product-mode fork.
  Auto selects resident MPSGraph only for supported f32 dirty-product work above
  its profitability guard; explicit Metal bypasses the small-batch guard, while
  unsupported or failed resident work uses the same CPU product path.
