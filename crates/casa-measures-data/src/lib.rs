...
mod tests {
    ...
    ///
    /// To update: `cargo run --example update_eop -p casa-measures-data --features update`
    /// then copy the new file to `crates/casa-measures-data/data/finals2000A.data`.
    #[test]
    fn bundled_data_not_stale() {
        ...
        assert!(
            ...
            "Bundled EOP data is {days_old:.0} days old (threshold: {BUNDLED_STALENESS_THRESHOLD_DAYS:.0} days). \
             Update with: cargo run --example update_eop -p casa-measures-data --features update \
             then copy to crates/casa-measures-data/data/finals2000A.data"
        );
    }
}
