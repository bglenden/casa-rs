...
case_dir="${external_root}/artifacts/experiments/casa-rs-aw-datagrid-tt0-arithmetic-compat-4096-full16-first-vb-v1"
...
validator="${experiment_dir}/vlass_aw_datagrid_tt0_arithmetic_compat_compare.py"
...
fail() {
    echo "VLASS casa-rs TT0 arithmetic compatibility: $*" >&2
    exit 1
}
...
mkdir "${case_dir}"
{
    printf 'schema\tcasa-rs-vlass-aw-datagrid-tt0-arithmetic-compat-provenance-v1\n'
...
"${timeout_command}" --signal=TERM --kill-after=15s 300s \
    ...
    CASA_RS_AW_TT0_ARITHMETIC_COMPAT_OUTPUT_V1="${receipt}" \
...
rg -F 'AWProject TT0 arithmetic-compatibility diagnostic stopped before production dispatch' \
    "${run_log}" >/dev/null ||
...
[[ "${comparison_status}" == "0" ]] ||
    fail "arithmetic-compatibility evidence was invalid; preserve ${case_dir}"
[[ -s "${comparison}" ]] ||
    fail "validator did not create ${comparison}; preserve ${case_dir}"
classification="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["comparison"]["classification"])' "${comparison}")"
printf 'comparison_sha256\t%s\n' "$(sha256 "${comparison}")" >>"${provenance}"
printf 'classification\t%s\n' "${classification}" >>"${provenance}"
printf 'completed_at_utc\t%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >>"${provenance}"
require_clean_checkout
printf 'result\tvalidly-classified-arithmetic-compatibility-evidence\n' >>"${provenance}"
echo "VLASS casa-rs TT0 arithmetic-compatibility evidence classified as ${classification}: ${comparison}"
