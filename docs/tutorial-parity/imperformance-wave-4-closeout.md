...
Verification: benchmark JSONs listed below; `python3 -m unittest tools/perf/imager/test_wave4_acceleration_matrix.py`; `python3 -m unittest tools/perf/imager/test_run_workload.py`; `cargo fmt --all -- --check`; `cargo clippy --workspace --all-targets -- -D warnings`; `cargo test -p casars-imager imaging_worker_plan --lib`; `cargo test -p casars-imager mosaic_cube_multiscale_slab_has_planned_metal_command_target --lib`; `cargo test -p casars-imager multiscale_metal_runtime_planner_sets_default_minor_cycle_chunk --lib`; `cargo test -p casars-imager mosaic --lib -- --test-threads=1`; `cargo test -p casa-imaging multiscale_metal_auto_chunk_planner_uses_shape_scaled_adaptive_chunks --lib`; `cargo test -p casa-imaging --lib -- --test-threads=1`; `docs/tutorial-parity/imperformance-wave-4-large-dirty-attempts.md`
...
evidence for W4-18 acceleration review. Brian reviewed the combined performance
and correctness page on 2026-07-02 and accepted the remaining clean-row
correctness/performance misses as Wave 4 residuals. The accepted misses remain
visible in the matrix rather than changing thresholds or hiding the underlying
measurements.
...
| Cubedata clean Hogbom | medium, 64 ch, 1024, niter=10000 | n/a | 262.735 | 208.483 | 1121.845 | 1.26x Metal vs CPU; 5.38x vs CASA | bad deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T051022Z-wave4-standard-cubedata-line-medium-clean-hogbom-c0c94fad` |
| Cubedata clean Clark | medium, 64 ch, 1024, niter=10000 | 159.595 | 43.880 | 39.625 | 632.483 | 3.64x multi-worker vs serial; 1.11x Metal vs CPU; 15.96x vs CASA | investigate deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T045233Z-wave4-standard-cubedata-line-medium-clean-clark-381407de` |
| Cubedata clean multiscale | medium, 64 ch, 1024, niter=10000 | n/a | 817.799 | 567.588 | 4401.249 | 1.44x Metal vs CPU; 7.75x vs CASA | bad deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T120835Z-wave4-standard-cubedata-line-medium-clean-multiscale-c4bc0f0d`; CASA/correctness row `20260618T133611Z-wave4-standard-cubedata-line-medium-clean-multiscale-c0d6a582` |
...
| Mosaic cube clean Hogbom | small, 8 ch, 512, niter=1000 | 10.128 | 7.115 | 7.108 | 6.697 | 1.42x multi-worker vs serial; 1.00x Metal vs CPU; 0.94x vs CASA | investigate deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T104849Z-wave4-mosaic-cube-alma-small-clean-hogbom-correctness-fe01c07f` |
| Mosaic cube clean Clark | small, 8 ch, 512, niter=1000 | 8.114 | 8.128 | 7.102 | 12.048 | 1.00x multi-worker vs serial; 1.14x Metal vs CPU; 1.70x vs CASA | bad deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T113427Z-wave4-mosaic-cube-alma-small-clean-correctness-1be23e89` |
| Mosaic cube clean multiscale | small, 8 ch, 512, niter=1000 | 15.188 | 11.153 | 7.116 | 35.323 | 1.36x multi-worker vs serial; 1.57x Metal vs CPU; 4.96x vs CASA | bad deep CASA comparison; accepted by Brian on 2026-07-02 for Wave 4 closeout | missed-accepted-by-Brian | `20260618T113551Z-wave4-mosaic-cube-alma-small-clean-multiscale-correctness-e3f3ed72` |
...
continues to cite the original selected performance row above. Brian explicitly
accepted living with this timing discrepancy on 2026-07-02, so the regenerated
row remains panel evidence rather than replacing the selected performance row.

Clean closeout rows that miss a required target or have bad/investigate
correctness are now accepted Wave 4 residuals by Brian signoff on 2026-07-02.
The matrix still requires clean correctness evidence to match the selected
clean iteration depth, so shallow `niter=2` CASA rows do not satisfy the deep
`niter=10000` multiscale or cubedata clean rows.
...
Standard cube and cubedata multiscale received one retained planning
optimization on 2026-06-18: the shared imaging worker planner now computes an
adaptive Metal multiscale command-buffer target from output shape, deconvolver
iteration count, scale count, and worker count, and standard cube, cubedata,
and mosaic cube slab paths all consume that common backend command target. For
...
rows remain accepted misses because the deep CASA comparisons are bad.
...
product writing. Standard cube multiscale spends about `5.0 s` in control
source reads, `0.3 s` in product writing, and `5664.6` worker-seconds in the
...
the auto/default runtime from `278.992 s` to `242.842 s` (`1.15x`). The current
review matrix now uses comparable small 512-pixel, niter=1000 rows for mosaic
clean performance and correctness. Those rows show useful Metal speedup for
multiscale (`1.57x` over multi-worker CPU) and CASA speedup for Clark and
multiscale, but Hogbom is still slower than CASA and the deep mosaic clean
correctness comparisons are investigate or bad. W4-21 is accepted for Wave 4
closeout by Brian signoff on 2026-07-02, with the small-row mosaic clean
correctness/performance misses left visible for follow-up work rather than
hidden or thresholded away.
...
- Clean Clark current niter=1000:
  `/Volumes/GLENDENNING/casa-rs-imperformance/_tmp_safe_to_delete/w4-21-clark-small-niter1000-after-divergence/20260618T113427Z-wave4-mosaic-cube-alma-small-clean-correctness-1be23e89.json`
- Clean multiscale current niter=1000:
  `/Volumes/GLENDENNING/casa-rs-imperformance/_tmp_safe_to_delete/w4-21-multiscale-small-niter1000-after-divergence/20260618T113551Z-wave4-mosaic-cube-alma-small-clean-multiscale-correctness-e3f3ed72.json`
