...
| CASA `outlierfile` | parse extra image definitions into compiled image-domain geometry | `casa-imaging-model` canonical domains; `casa-ms` selected GridFT projections; shared reconstruction/runtime/product route; application parser | T31/#517 replaces the displaced per-image runner with one bounded selected-observation traversal, summed prediction, domain adjoints, deterministic CASA field-local Högbom controllers, atomic model delta, and domain-resolved products. The frozen `refim_twopoints_twochan.ms` gate covers exact WCS plus dirty/clean PSF, residual, model, and image at NRMS <= `1e-3`; the worst accepted result is `1.682477e-6`. Unsupported mask forms, cube/MTMFS/W-projection combinations, and non-standard gridders/deconvolvers reject during planning. |
