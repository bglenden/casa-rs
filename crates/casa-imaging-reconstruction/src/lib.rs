...
use casa_imaging_model::{
    ...
    ModelPolarizationConversionRegistry, ModelReprojectionPolicy, ModelSample, ModelSourceShape,
    ModelSupport, ModelUncoveredTargetPolicy, ModelValue, NumericPrecision, PolarizationCoordinate,
    ReconstructionBasis,
};
...
const REPROJECTION_DOMAIN: &[u8] = b"casa-rs-model-reprojection";
const REPROJECTION_VERSION: u32 = 2;
const SUPPORT_DOMAIN: &[u8] = b"casa-rs-model-support";
...
pub struct PreparedReprojectedSeed {
    ...
    reprojection: ModelReprojectionId,
...
impl PreparedReprojectedSeed {
    ...
    pub fn lifecycle_requirements(&self) -> ModelLifecycleRequirements {
        ModelLifecycleRequirements::new(
            ...
            ModelInputCommitment::ReprojectedSeed {
                source: self.source,
                source_shape: Box::new(self.source_shape.clone()),
                contract: self.preparation_contract,
                reprojection: self.reprojection.identity(),
    ...
    #[must_use]
    pub const fn reprojection_id(&self) -> ModelReprojectionId {
        self.reprojection
    }
...
impl ModelLifecycle {
    ...
    pub fn ingest_reprojected(
        &self,
        prepared: PreparedReprojectedSeed,
    ) -> Result<ModelGeneration, ModelLifecycleError> {
        self.ensure_open()?;
        let ModelInputCommitment::ReprojectedSeed {
            source: expected_source,
            source_shape,
            contract,
            reprojection,
        ...
        if prepared.source != *expected_source
            || prepared.source_shape != **source_shape
            || prepared.target_shape != *self.contract.target()
            || prepared.preparation_contract != *contract
        ...
        if prepared.support != *support
            || model_support_identity(prepared.samples.iter().map(|sample| sample.support()))
                != *support
        {
            return Err(ModelLifecycleError::SupportIdentityMismatch);
        }
        if prepared.reprojection.identity() != *reprojection {
        ...
        self.mint_generation(
            ...
            ModelGenerationOrigin::Ingested {
                ...
                reprojection: Some(prepared.reprojection),
            },
        )
    }
...
/// Compute the canonical validity-mask identity used by aligned seed commitments.
#[must_use]
pub fn model_support_identity(support: impl IntoIterator<Item = ModelSupport>) -> LogicalIdentity {
    let mut encoder = Encoder::new(SUPPORT_DOMAIN, SUPPORT_VERSION);
...
pub fn prepare_reprojected_seed<R: ModelSourceReader>(
    ...
    let mut encoder = Encoder::new(REPROJECTION_DOMAIN, REPROJECTION_VERSION);
    encoder.identity(source_shape.identity().as_bytes());
    encoder.identity(target_shape.identity().as_bytes());
    encoder.usize(target_shape.sample_count());
    ...
    for target_index in 0..target_shape.sample_count() {
        ...
        match stencil {
            ...
            Some(stencil) => {
                ...
                encoder.u8(1);
                encoder.usize(stencil.len());
                let mut valid = true;
                let mut value = 0.0_f64;
                for weighted in stencil {
                    let source_index = source_shape
                        .flat_index(weighted.cell)
                        .expect("owner-derived source stencil remains in range");
                    encoder.usize(source_index);
                    encoder.u64(canonical_f64_bits(weighted.weight));
                    let sample = reader
    ...
    Ok(PreparedReprojectedSeed {
        ...
        bounds,
        precision,
        reprojection: ModelReprojectionId(LogicalIdentity::from_sha256(encoder.finish())),
        support,
        samples: samples.into_boxed_slice(),
    })
}
