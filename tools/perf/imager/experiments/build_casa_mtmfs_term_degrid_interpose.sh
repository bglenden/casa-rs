...
output="${CASA_MTMFS_TERM_DEGRID_INTERPOSER:-${TMPDIR:-/tmp}/casa_mtmfs_term_degrid_interpose.dylib}"
...
get_symbol="__ZN4casa5refim14MultiTermFTNew3getERNS_2vi10VisBuffer2Ei"
modify_symbol="__ZN4casa5refim14MultiTermFTNew14modifyModelVisERNS_2vi10VisBuffer2Ej"
finalize_symbol="__ZN4casa5refim14MultiTermFTNew13finalizeToVisEv"

fail() {
    echo "CASA MT-MFS term-degrid interposer build: $*" >&2
    exit 1
}
...
[[ -f "${exact_casatools}/src/code/synthesis/TransformMachines2/MultiTermFTNew.h" ]] ||
    fail "missing exact refim MultiTermFTNew header"
...
[[ -f "${local_casatools}/include/casacore/casa/config.h" ]] ||
    fail "missing generated casacore config header"
[[ -f "${official_synthesis}" ]] ||
    fail "missing official CASA synthesis dylib: ${official_synthesis}"

symbols="$(nm -gU "${official_synthesis}")"
for symbol in "${get_symbol}" "${modify_symbol}" "${finalize_symbol}"; do
    grep -Fq "${symbol}" <<<"${symbols}" ||
        fail "official CASA dylib does not export expected symbol ${symbol}"
done
...
clang++ \
    ...
    "${experiment_dir}/casa_mtmfs_term_degrid_interpose.cc" \
    "${official_synthesis}" \
...
grep -Fq "_casa_mtmfs_term_degrid_oracle_ready_v1" <<<"${built_exports}" ||
    fail "built interposer does not export its v1 readiness marker"
built_bindings="$(nm -m "${output}")"
for symbol in "${get_symbol}" "${modify_symbol}" "${finalize_symbol}"; do
    grep -Fq "${symbol} (from libcasacpp_synthesis)" <<<"${built_bindings}" ||
        fail "interposed import is not two-level-bound to CASA synthesis: ${symbol}"
done
...
grep -Fq "size 0x0000000000000020" <<<"${interpose_layout}" ||
    fail "dyld interpose section does not contain exactly two arm64 entries"
