...
fn read_scalar_at(
    ...
    dt: CasacoreDataType,
    big_endian: bool,
) -> Result<casa_types::Value, StorageError> {
...
fn read_array_at(
    ...
    let arr_val = match dt {
        ...
        CasacoreDataType::TpComplex => {
            let v: Vec<casa_types::Complex32> = (0..nrelem)
                .map(|i| {
                    let (re, im) = if big_endian {
                    ...
                    casa_types::Complex32::new(re, im)
                })
                .collect();
        ...
        CasacoreDataType::TpDComplex => {
            let v: Vec<casa_types::Complex64> = (0..nrelem)
                .map(|i| {
                    let (re, im) = if big_endian {
                    ...
                    casa_types::Complex64::new(re, im)
                })
                .collect();
...
fn read_ism_scalar_columns_rows_uncached(
    ...
    let col_info: Vec<Option<CasacoreDataType>> = owned_col_descs
        .iter()
        .map(|col_desc| {
            if col_desc.is_array || col_desc.is_record() {
                return Ok(None);
            }
            Ok(Some(CasacoreDataType::from_primitive_type(
    ...
    for (row_index, out_idx) in requests {
        ...
        if cached_interval != Some(interval_idx) {
            ...
            let rows_in_bucket = bucket_end.saturating_sub(cached_bucket_start);
            cached_bucket_values = owned_col_descs
                .iter()
                .zip(col_info.iter())
                .enumerate()
                .map(|(col_idx, (col_desc, scalar_dt))| match scalar_dt {
                    Some(scalar_dt) => load_scalar_bucket_column_values(
                        &bucket,
                        col_desc,
                        col_idx,
                        *scalar_dt,
                    ...
                    None => Ok(vec![None; rows_in_bucket]),
                })
...
fn default_value(
    ...
    match dt {
        ...
        CasacoreDataType::TpComplex => {
            Value::Scalar(ScalarValue::Complex32(casa_types::Complex32::new(0.0, 0.0)))
        }
        CasacoreDataType::TpDComplex => {
            Value::Scalar(ScalarValue::Complex64(casa_types::Complex64::new(0.0, 0.0)))
        }
...
fn values_to_column_raw_scalar(
    ...
    match dt {
        ...
        CasacoreDataType::TpComplex => {
            let v: Vec<casa_types::Complex32> = values
                .iter()
                .map(|v| match v {
                    Value::Scalar(ScalarValue::Complex32(c)) => *c,
                    _ => casa_types::Complex32::new(0.0, 0.0),
                })
                .collect();
            Ok(ColumnRawData::Complex32(v))
        }
        CasacoreDataType::TpDComplex => {
            let v: Vec<casa_types::Complex64> = values
                .iter()
                .map(|v| match v {
                    Value::Scalar(ScalarValue::Complex64(c)) => *c,
                    _ => casa_types::Complex64::new(0.0, 0.0),
                })
                .collect();
            ...
            for v in values {
                if let Value::Array(ArrayValue::Complex32(arr)) = v {
                    ...
                    flat.extend(std::iter::repeat_n(
                        casa_types::Complex32::new(0.0, 0.0),
                        nrelem,
                    ));
                }
            }
        ...
        CasacoreDataType::TpDComplex => {
            ...
            for v in values {
                if let Value::Array(ArrayValue::Complex64(arr)) = v {
                    ...
                    flat.extend(std::iter::repeat_n(
                        casa_types::Complex64::new(0.0, 0.0),
                        nrelem,
                    ));
                }
            }
...
pub(crate) fn write_ism_file_scalar_column_sources(
    ...
    file.write_all(&vec![0u8; ISM_HEADER_SIZE as usize])?;
    let mut nr_buckets = 0usize;

    let write_bucket = |file: &mut File, bucket: &BucketBuilder| -> Result<(), StorageError> {
        ...
        let index_len: usize = bucket
            .col_indices
            .iter()
            .map(|(row_nrs, _)| 4 + row_nrs.len() * 8)
            .sum();
        if 4 + data_len + index_len > raw.len() {
            return Err(StorageError::FormatMismatch(format!(
                "ISM bucket requires {} bytes, exceeds planned bucket size {}",
                4 + data_len + index_len,
                raw.len()
            )));
        }
        let index_start = raw.len() - index_len;
        ...
        for (row_nrs, offsets) in &bucket.col_indices {
            let count = row_nrs.len() as u32;
            let encoded_count = if big_endian {
                count.to_be_bytes()
            } else {
                count.to_le_bytes()
            };
            raw[cursor..cursor + 4].copy_from_slice(&encoded_count);
            cursor += 4;
            for &row in row_nrs {
                ...
                raw[cursor..cursor + 4].copy_from_slice(&encoded);
                cursor += 4;
            }
    ...
    if nrrow > 0 {
        ...
        for row_idx in 0..nrrow {
            ...
            for col_idx in 0..ncol {
                ...
                row_encoded_values.push(encoded);
            }
            ...
            if rel_row > 0 && total_estimate > bucket_size as usize {
                ...
                for (col_idx, encoded) in row_encoded_values.into_iter().enumerate() {
...
mod tests {
    ...
    fn string_encoding_round_trip() {
        for big_endian in [true, false] {
            let val =
                casa_types::Value::Scalar(casa_types::ScalarValue::String("hello".to_string()));
    ...
    fn string_encoding_empty() {
        for big_endian in [true, false] {
            let val = casa_types::Value::Scalar(casa_types::ScalarValue::String(String::new()));
            let encoded = encode_string_value(Some(&val), 1, big_endian);
            let decoded = read_ism_string(&encoded, 1, big_endian).unwrap();
    ...
    fn bool_encoding_round_trip() {
        for big_endian in [true, false] {
            let val_true = casa_types::Value::Scalar(casa_types::ScalarValue::Bool(true));
            let val_false = casa_types::Value::Scalar(casa_types::ScalarValue::Bool(false));
            let enc_true =
    ...
    #[test]
    fn selected_scalar_reader_ignores_array_siblings_owned_by_same_manager() {
        use casa_types::{PrimitiveType, RecordField, RecordValue, ScalarValue, Value};
        use tempfile::TempDir;

        let scalar_desc = ColumnDescContents {
            class_name: "ScalarColumnDesc".to_string(),
            col_name: "FIELD_ID".to_string(),
            comment: String::new(),
            data_manager_type: "IncrementalStMan".to_string(),
            data_manager_group: "ISMData".to_string(),
            data_type: CasacoreDataType::TpInt,
            option: 0,
            nrdim: 0,
            shape: vec![],
        ...
        let physical_sibling_desc = ColumnDescContents {
            col_name: "UVW".to_string(),
            ..scalar_desc.clone()
        };
        let rows = [0, 0, 2, 2]
            .into_iter()
            .map(|field_id| {
                RecordValue::new(vec![
                    ...
                    RecordField::new("UVW", Value::Scalar(ScalarValue::Int32(17))),
                ])
            })
            .collect::<Vec<_>>();
        let dir = TempDir::new().expect("tempdir");
        let file_path = dir.path().join("mixed-manager.ism");
        let dm_blob = write_ism_file(
            &file_path,
            &[scalar_desc.clone(), physical_sibling_desc.clone()],
            &rows,
            false,
        )
        .expect("write ISM fixture");

        let array_sibling_desc = ColumnDescContents {
            ...
            ..physical_sibling_desc
        };
        let selected = read_ism_scalar_columns_rows_uncached(
            &file_path,
            &dm_blob,
            &[&scalar_desc, &array_sibling_desc],
            &[3, 0, 2],
        )
        .expect("read scalar column beside array sibling");
        ...
        assert_eq!(selected[1], vec![None, None, None]);
    }
}
