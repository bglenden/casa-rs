...
source_major_residual_compensation="${CASA_RS_VLASS_SOURCE_MAJOR_RESIDUAL_COMPENSATION:-0}"
...
if [[ "$source_major_full_compensation" == "1" && "$source_major_residual_compensation" == "1" ]]; then
    echo "source-major full and residual-only compensation are mutually exclusive" >&2
    exit 2
...
if [[ "$source_major_residual_compensation" == "1" ]]; then
    label="${label}-source-major-residual-compensation"
    experimental_environment+=(
        CASA_RS_EXPERIMENTAL_AWPROJECT_SOURCE_MAJOR_RESIDUAL_COMPENSATION=1
    )
elif [[ "$source_major_residual_compensation" != "0" ]]; then
    echo "CASA_RS_VLASS_SOURCE_MAJOR_RESIDUAL_COMPENSATION must be 0 or 1" >&2
    exit 2
fi
