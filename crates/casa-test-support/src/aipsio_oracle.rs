...
//! Rust-facing AIPSIO cross-matrix oracle.
...
#[derive(Debug, Clone)]
pub(crate) struct PreparedPrimitiveCase {
    pub(crate) wire_value: Value,
    pub(crate) expected_value: Value,
    pub(crate) original_shape: Option<Vec<usize>>,
}

pub(crate) fn prepare_primitive_case(
    value: &Value,
) -> Result<PreparedPrimitiveCase, AipsIoCrossError> {
    match value {
...
pub(crate) fn restore_decoded_shape(
    decoded: Value,
...
pub(crate) fn flatten_array_value_fortran(array: &ArrayValue) -> ArrayValue {
    match array {
...
pub(crate) fn reshape_array_value_from_fortran(
    array: ArrayValue,
    shape: &[usize],
...
pub(crate) fn flatten_ndarray_fortran<T: Clone>(array: &ArrayD<T>) -> Vec<T> {
...
pub(crate) fn reshape_from_fortran<T: Clone>(
    fortran_values: &[T],
    shape: &[usize],
...
pub(crate) fn unravel_fortran_index(mut linear: usize, shape: &[usize]) -> Vec<usize> {
...
pub(crate) fn unravel_c_index(mut linear: usize, shape: &[usize]) -> Vec<usize> {
...
pub(crate) fn ravel_fortran_index(idx: &[usize], shape: &[usize]) -> usize {
...
fn cpp_encode_value(value: &Value, byte_order: ByteOrder) -> Result<Vec<u8>, String> {
    ...
    unsafe {
        CasacoreOracleRuntime::owned_vec(
            ...
            out_ptr,
            out_len,
            casacore_cpp_aipsio_free_bytes,
        )
    }
    .map_err(|error| error.to_string())
}
...
fn cpp_decode_value(
    ...
    let offsets = unsafe {
        CasacoreOracleRuntime::owned_vec(
            "aipsio.decode offsets",
            out_offsets_ptr,
            out_offsets_len,
            casacore_cpp_aipsio_free_offsets,
        )
    }
    .map_err(|error| error.to_string())?;
