...
fn rc_rust_temp_image_materializes_cpp_reads_metadata() {
    ...
    assert_eq!(ImageOracle::read_image_shape(&path).unwrap(), shape_i32);
    assert_eq!(
        ImageOracle::read_image_data(&path, data.len()).unwrap(),
        data
    );
    assert_eq!(ImageOracle::read_image_units(&path).unwrap(), "K");
    ...
    assert_eq!(
        ImageOracle::read_image_default_mask(&path, mask.len()).unwrap(),
        mask
    );
    assert_eq!(
        ImageOracle::read_image_info_object_name(&path).unwrap(),
        "RustTemp"
    );
    assert_eq!(
        ImageOracle::read_image_info_type(&path).unwrap(),
        "Intensity"
    );
}
...
fn cr_cpp_temp_image_materializes_rust_reads_metadata() {
    ...
    ImageOracle::create_temp_image_materialized(&path, &shape_i32, &data, "Jy", "CppTemp", "Beam")
        .unwrap();
