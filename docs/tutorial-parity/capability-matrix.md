...
| `tclean` MFS | partial/available | `casa-imaging`, `casars-imager` | `casars-imager`; Python projection includes automask and `savemodel=modelcolumn` controls | tutorial parameter mapping, startmodel/outlier orchestration tails | CASA `tclean` image products plus MODEL_DATA comparisons for `savemodel=modelcolumn` | imager parity scripts and tutorial timings | #117, #127, #167, #196 |
