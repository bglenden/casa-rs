...
| CASA `startmodel` | seed `imagename.model` from one or more model images before deconvolution/model prediction | explicitly unsupported in the strict task JSON surface; no CLI/Python projection | deferred to #219 because CASA routes it through image-store restart/model-copy semantics (`casatasks/src/private/task_deconvolve.py`, `casatools/src/code/synthesis/ImagerObjects/SynthesisUtilMethods.cc`, `SynthesisImager.cc::createImStore`) that need a focused model-seeding follow-up |
| CASA `outlierfile` | parse and orchestrate extra image definitions / outlier fields | explicitly unsupported in the strict task JSON surface; no CLI/Python projection | deferred to #220 because CASA treats this as a multi-image/outlier-field orchestration path (`casatasks/src/private/imagerhelpers/_gclean.py`), which is broader than the current single-image frontend |
...
| CASA `nmajor` / `fullsummary` | major-cycle limit and returned minor-cycle summary detail | native run reports expose `major_cycles`, `minor_iterations`, stop reason, and per-channel summaries; no CASA `nmajor` knob yet | #196 records the source seam (`imager_base.py::getSummary`, `SIMinorCycleController`, `_gclean.py` nmajor docs); explicit `nmajor` limiting remains deferred to #221 until a tutorial/product route needs CASA-compatible task return shape |
