...
const MODEL_LIFECYCLE_CONTRACT_IDENTITY_VERSION: u32 = 2;
...
pub enum ModelInputCommitment {
    ...
    ReprojectedSeed {
        /// Immutable source artifact identity.
        source: LogicalIdentity,
        /// Exact source WCS, basis, polarization, and shape.
        source_shape: Box<ModelSourceShape>,
        /// Compiler-owned coordinate, numerics, and support-rule commitment.
        contract: LogicalIdentity,
        /// Identity of the canonical target-ordered mapping.
        reprojection: LogicalIdentity,
        /// Identity of the owner-derived projected target validity mask.
        support: LogicalIdentity,
    },
...
fn validate_input_commitment(
    ...
    let matches = match (model, input) {
        ...
        (
            ...
            if *contract != preparation_contract {
                return Err(ModelContractError::ReprojectionContractMismatch);
            }
            expected == *source
                && contract.as_bytes() != [0; 32]
                && identities_are_set(&[*source, *reprojection, *support])
                && source_shape.as_ref() != target
        }
...
fn model_lifecycle_contract_id(
    ...
    encoder.digest(numerics.as_bytes());
    encoder.digest(reprojection.as_bytes());
    ...
    encode_input_commitment(&mut encoder, input);
    ModelLifecycleContractId(LogicalIdentity::from_sha256(encoder.finish()))
}

fn encode_input_commitment(encoder: &mut CanonicalEncoder, input: &ModelInputCommitment) {
    match input {
        ModelInputCommitment::Empty => encoder.u8(0),
        ModelInputCommitment::AlignedSeed { source, support } => {
            encoder.u8(1);
        ...
        ModelInputCommitment::ReprojectedSeed {
            ...
            encoder.u8(2);
            encoder.identity(*source);
            source_shape.encode(encoder);
            encoder.digest(contract.as_bytes());
