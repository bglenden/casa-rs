...
use crate::{
    ...
    measurement_equation::{
        VisibilityInnerProduct, encode_spectral_sampling, encode_visibility_inner_product,
    },
    observation::{
...
pub(crate) fn inspect_selected_observation(
    ...
    let mut source = SourceInspection::new(&expected_sources[source_index]);
    let mut generation = SelectedObservationGenerationEncoder::new();

    for sample in samples {
        while sample.address.measurement_set != source.expected.measurement_set() {
            if source.observed_row_count() == 0
                && source.expected.selection().rows().selected_row_count() > 0
            {
                return Err(
            ...
            source.finish()?;
            source_index += 1;
            let Some(expected) = expected_sources.get(source_index) else {
                return Err(
                    SelectedObservationInspectionError::NonCanonicalSampleOrder {
                        measurement_set: sample.address.measurement_set,
                        physical_row: sample.address.physical_row,
            ...
            source = SourceInspection::new(expected);
        }
        let expected_prediction_target = if write_set
            .model_columns()
            .iter()
            .any(|write| write.measurement_set() == source.expected.measurement_set())
        {
            SelectedPredictionTarget::ModelData
        } else {
            SelectedPredictionTarget::NotRequested
        };
        source.push(&sample, expected_prediction_target)?;
        generation.push(&sample);
    }

    source.finish()?;
    for expected in &expected_sources[source_index + 1..] {
        SourceInspection::new(expected).finish()?;
    }
    Ok(generation.finish())
}
