...
fn save_bench_table(n: usize) -> (tempfile::TempDir, std::path::PathBuf) {
    ...
    table
        .save(casa_tables::TableOptions::new(&path))
...
fn save_varshape_bench_table(n: usize) -> (tempfile::TempDir, std::path::PathBuf) {
    ...
    let mut table = casa_tables::Table::with_schema(schema);
    for i in 0..n {
...
fn perf_varshape_read() {
    ...
    let table = casa_tables::Table::open(casa_tables::TableOptions::new(&path))
