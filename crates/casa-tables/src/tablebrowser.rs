...
pub use casars_tablebrowser_protocol::BrowserView as TableBrowserView;
...
impl TableBrowser {
    ...
    pub fn handle_request(
        &mut self,
        request: casars_tablebrowser_protocol::BrowserRequestEnvelope,
    ) -> BrowserResponseEnvelope {
        if request.version != casars_tablebrowser_protocol::PROTOCOL_VERSION {
            return BrowserResponseEnvelope::error(
                "unsupported_version",
                format!(
                    "expected protocol version {}, received {}",
                    casars_tablebrowser_protocol::PROTOCOL_VERSION,
                    request.version
                ),
            );
        }
...
mod tests {
    ...
    use casa_types::{ArrayD, Complex32, Complex64, RecordField, RecordValue, ScalarValue, Value};
    use casars_tablebrowser_protocol::{BrowserCommand, BrowserRequestEnvelope};
    ...
    fn browser_request_envelope_round_trips_through_handle_request() {
        ...
        match response.response {
            casars_tablebrowser_protocol::BrowserResponse::Snapshot(snapshot) => {
