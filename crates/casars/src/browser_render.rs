...
//! UI-neutral raster rendering for image-browser plane and spectrum surfaces.
...
use plotters::prelude::{FontTransform, PathElement, RGBColor, Rectangle, Text};
...
/// Theme palette used by the image-browser raster renderer.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum BrowserRenderTheme {
    DenseAnsi,
    RichPanel,
}
#[derive(Debug, Clone, PartialEq)]
pub struct ImagePlaneOverlayMarker {
    pub sample: (usize, usize),
    pub color_index: usize,
}

#[derive(Debug, Clone, PartialEq)]
pub struct ImagePlaneRenderInput {
    pub cache_key: u64,
...
#[derive(Debug, Clone, PartialEq)]
pub struct ImageSpectrumOverlaySeries {
    pub label: String,
...
#[derive(Debug, Clone, PartialEq)]
pub struct ImageSpectrumRenderInput {
    pub cache_key: u64,
    pub profile: ImageProfilePayload,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ImagePlaneDrawGeometry {
    pub x: u32,
    pub y: u32,
    pub width: u32,
    pub height: u32,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ImageSpectrumLayout {
...
pub fn render_image_plane_image(
    width: u32,
    height: u32,
    ...
    let marker_halo = match input.theme_mode {
        BrowserRenderTheme::DenseAnsi => [0, 0, 0],
        BrowserRenderTheme::RichPanel => [15, 23, 42],
    };
    let marker_center = match input.theme_mode {
        BrowserRenderTheme::DenseAnsi => [0, 0, 0],
        BrowserRenderTheme::RichPanel => [248, 250, 252],
    };
    ...
    draw_plane_frame_label(&mut image, geometry, input)?;
...
pub fn render_image_spectrum_image(
    width: u32,
    height: u32,
    ...
    let has_spectrum_values = spectrum_value_range(input).is_some();
    let (mut min_value, mut max_value) = spectrum_value_range(input).unwrap_or((-1.0, 1.0));
...
pub fn image_plane_draw_geometry(
    width: u32,
    height: u32,
...
pub fn image_spectrum_layout(width: u32, height: u32) -> ImageSpectrumLayout {
    let width = width.max(1);
    let height = height.max(1);
...
fn image_plane_axis_colors(theme_mode: BrowserRenderTheme) -> ([u8; 3], [u8; 3]) {
    match theme_mode {
        BrowserRenderTheme::DenseAnsi => ([96, 96, 96], [224, 224, 224]),
        BrowserRenderTheme::RichPanel => ([100, 116, 139], [226, 232, 240]),
    }
}
...
fn draw_plane_frame_label(
    ...
    if geometry.width < 48 || geometry.height < 24 {
        return Ok(());
    }

    let (badge_fill, badge_border, badge_text) = match input.theme_mode {
        BrowserRenderTheme::DenseAnsi => (
            RGBColor(8, 12, 20),
            RGBColor(96, 96, 96),
    ...
    let area =
        BitMapBackend::with_buffer(image.as_mut(), (image_width, image_height)).into_drawing_area();
    let font_size = plane_tick_font_size(geometry).max(12);
    let char_width = (font_size.max(8) as u32 / 2).max(6);
    let badge_width = (label.chars().count() as u32)
        .saturating_mul(char_width)
        .saturating_add(18)
        .min(geometry.width.saturating_sub(8))
        .max(24);
    let badge_height = (font_size as u32)
        .saturating_add(10)
        .min(geometry.height.saturating_sub(8));
    if badge_width == 0 || badge_height == 0 {
        return Ok(());
    }
    let badge_x = geometry.x.saturating_add(6);
    let badge_y = geometry.y.saturating_add(6);

    area.draw(&Rectangle::new(
        [
            (badge_x as i32, badge_y as i32),
            (
                badge_x.saturating_add(badge_width).saturating_sub(1) as i32,
                badge_y.saturating_add(badge_height).saturating_sub(1) as i32,
            ),
        ],
        badge_fill.filled(),
    ))
    ...
    area.draw(&Text::new(
        label.to_string(),
        (
            badge_x.saturating_add(8) as i32,
            badge_y.saturating_add(badge_height / 2) as i32,
        ),
...
fn plane_tick_font_size(geometry: ImagePlaneDrawGeometry) -> i32 {
    (geometry.width.min(geometry.height) / 28).clamp(11, 16) as i32
}

fn plane_axis_font_size(geometry: ImagePlaneDrawGeometry) -> i32 {
    (geometry.width.min(geometry.height) / 22).clamp(12, 18) as i32
}

fn plane_tick_length(geometry: ImagePlaneDrawGeometry) -> u32 {
    (geometry.width.min(geometry.height) / 30).clamp(6, 10)
}
...
fn plane_scale_title_font_size(geometry: ImagePlaneDrawGeometry) -> i32 {
    (geometry.width.min(geometry.height) / 24).clamp(11, 16) as i32
}

fn image_browser_highlight(theme_mode: BrowserRenderTheme) -> [u8; 3] {
    match theme_mode {
        BrowserRenderTheme::DenseAnsi => [255, 208, 96],
        BrowserRenderTheme::RichPanel => [196, 64, 32],
    }
}
...
fn image_probe_overlay_color(theme_mode: BrowserRenderTheme, color_index: usize) -> [u8; 3] {
    const DENSE: &[[u8; 3]] = &[
        [255, 96, 96],
        [255, 224, 64],
        [255, 96, 220],
    ...
    let palette = match theme_mode {
        BrowserRenderTheme::DenseAnsi => DENSE,
        BrowserRenderTheme::RichPanel => RICH,
    };
    palette[color_index % palette.len()]
}

fn image_region_overlay_color(theme_mode: BrowserRenderTheme) -> [u8; 3] {
    match theme_mode {
        BrowserRenderTheme::DenseAnsi => [255, 255, 0],
        BrowserRenderTheme::RichPanel => [255, 82, 82],
    }
}
...
#[allow(clippy::too_many_arguments)]
fn draw_selected_spectrum_marker(
    image: &mut RgbImage,
    profile: &ImageProfilePayload,
    plot_left: u32,
    plot_top: u32,
    ...
    min_value: f64,
    max_value: f64,
    has_spectrum_values: bool,
    guide_line: Rgb<u8>,
    marker: Rgb<u8>,
) {
    let Some(selected) = profile.samples.get(profile.selected_sample_index) else {
        return;
    };
    let x_value = profile_sample_x_value(selected);
    let marker_x = plot_value_x(x_value, x_domain.0, x_domain.1, plot_left, plot_width);
    let marker_y = if !selected.masked && selected.finite && selected.value.is_finite() {
        plot_value_y(selected.value, min_value, max_value, plot_top, plot_height)
    } else {
        plot_value_y(0.0, min_value, max_value, plot_top, plot_height)
    };
    draw_vertical_line(
        image,
        marker_x,
    ...
    let radius = if has_spectrum_values { 3 } else { 2 };
    draw_cross(image, marker_x, marker_y, radius, marker);
}
...
mod tests {
    use super::{
        BrowserRenderTheme, ImagePlaneColormap, ImagePlaneDrawGeometry, ImagePlaneRenderInput,
        ImageSpectrumRenderInput, image_plane_draw_geometry, plane_axis_font_size,
        plane_tick_font_size, plane_tick_length, render_image_plane_image,
        render_image_spectrum_image, spectrum_axis_font_size, spectrum_tick_font_size,
        spectrum_tick_length,
    };
    use casars_imagebrowser_protocol::{
    ...
    fn image_plane_render_highlights_selected_cell() {
        let image = render_image_plane_image(
            ...
            &ImagePlaneRenderInput {
                ...
                invert: false,
                theme_mode: BrowserRenderTheme::DenseAnsi,
            },
        )
    ...
    #[test]
    fn spectrum_typography_matches_plane_typography() {
        let geometry = ImagePlaneDrawGeometry {
            x: 0,
            y: 0,
            width: 720,
            height: 180,
        ...
        assert_eq!(
            spectrum_tick_font_size(geometry),
            plane_tick_font_size(geometry)
        );
        assert_eq!(
            spectrum_axis_font_size(geometry),
            plane_axis_font_size(geometry)
        );
        assert_eq!(spectrum_tick_length(geometry), plane_tick_length(geometry));
    }
    ...
    #[test]
    fn image_spectrum_render_marks_selected_sample_when_all_values_masked() {
        let mut profile = ImageProfilePayload {
            axis: 2,
            axis_name: "Frequency".into(),
            axis_unit: "Hz".into(),
            value_unit: "Jy/beam".into(),
        ...
        let render = |selected_sample_index: usize, profile: &ImageProfilePayload| {
            let mut profile = profile.clone();
            profile.selected_sample_index = selected_sample_index;
            ...
            .expect("render masked image spectrum")
            .to_rgb8()
        };

        let first = render(0, &profile);
        let second = render(2, &profile);

        assert_ne!(first.as_raw(), second.as_raw());
        let guide_pixels = first
            .pixels()
            .filter(|pixel| pixel.0 == [255, 208, 96])
            .count();
        assert!(guide_pixels >= 3);

        profile.selected_sample_index = 2;
        let third = render(profile.selected_sample_index, &profile);
        assert_eq!(second.as_raw(), third.as_raw());
    }
}
