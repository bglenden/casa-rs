...
| Residual-only later major passes | CASA separates PSF construction (`dopsf=true`) from ordinary model-degrid/residual major cycles (`dopsf=false`) (`SynthesisImagerVi2.cc:1392-1428`, `1447-1475`, `1488-1489`). The last optimized pre-cutover implementation had dedicated bounded residual-refresh construction and accumulation (`fff9c2...:crates/casa-imaging/src/execution.rs:870-969`, `7332-7388`). | Candidate `70d823603` transplanted invariant normal-state reuse into reconstruction/runtime and reduced seven one-channel later passes by about 73 ms each, but total serial wall was unchanged at 84.818 s versus 84.813 s. | Reconstruction for scientific invariants; runtime for carried lifetime and residency. | **Reject as a standalone Delivery 1 optimization.** Correct products and lower later-pass work did not move terminal wall on the required discriminator; preserve the historical mechanism as evidence for a future profile that proves it material. |
