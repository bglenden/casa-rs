...
emit_rust_backend_diagnostics() {
  ...
  grep -E \
    '^(single_plane_execution_plan|standard_mfs_runtime_plan|standard_mfs_memory_plan_actual|visibility_source_stream_consumer|standard_mfs_profile_run|spectral_slab_plan|spectral_slab_event|spectral_slab_memory|cube_per_plane_backend_summary|cube_slab_executor_limitation|cube_source_row_blocks|cube_shared_(direct_)?plane_executor_summary|cube_shared_direct_dirty_eligibility|cube_shared_direct_dirty_source|independent_plane_executor_owned_streaming_done|frontend stage=(prepare_plane_input/(data_coverage|accumulate_rows/detail|finish_cube_source_row_blocks)|write_products|cube_slab/|cli/))' \
    "$stderr_file" || true
}
