...
# One-shot launcher for the casa-rs native-component V3 diagnostic.  The
...
case_name="casa-rs-aw-datagrid-native-components-4096-full16-first-vb-v3"
...
controlled_stop_marker="AWProject native-components-v3 audit stopped before casa-imaging core/CF/cache/grid dispatch"

fail() {
    echo "VLASS casa-rs native-component V3 audit: $*" >&2
    exit 1
}
...
mkdir "${case_dir}"
{
    printf 'schema\tcasa-rs-vlass-aw-native-components-provenance-v3\n'
    printf 'case_name\t%s\n' "${case_name}"
...
"${timeout_command}" --signal=TERM --kill-after=15s 300s \
    ...
    CASA_RS_AW_NATIVE_COMPONENTS_AUDIT_OUTPUT_V3="${receipt}" \
...
printf 'result\tvalidly-classified-native-components-v3\n' >>"${provenance}"
require_clean_checkout
require_exact_checkpoint
echo "VLASS casa-rs native-component V3 evidence classified as ${classification}: ${comparison}"
