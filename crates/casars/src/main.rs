...
fn main() {
    ...
    tracing::info!("casars started");
    if let Err(error) = casars::run_with_cli_args(args) {
        tracing::error!(casa.priority = "SEVERE", error = %error, "casars failed");
        eprintln!("Error: {error}");
        let _ = logging_guard.flush();
        std::process::exit(1);
    }
    tracing::info!("casars completed");
