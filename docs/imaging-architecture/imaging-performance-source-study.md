...
| Residual-only later major passes | CASA separates PSF construction (`dopsf=true`) from an ordinary major cycle (`dopsf=false`); only the latter initializes degridding and predicts the model before gridding (`SynthesisImagerVi2.cc:1392-1428`, `1447-1475`, `1488-1489`). The last optimized pre-cutover implementation likewise had a dedicated bounded `build_for_residual_refresh` and `accumulate_residual_grid` path (`fff9c2...:crates/casa-imaging/src/execution.rs:870-969`, `7332-7388`). | Before the #540 candidate, every later major replay rebuilt dirty, PSF, sensitivity, and sum-weight state even though reconciliation discarded the new dirty plane in favor of the exact residual. | Reconstruction owns invariant normal state and the residual kernel; runtime owns its carried lifetime and exact residency. | **Adapt.** Carry the prior PSF, sensitivity, sum weights, validity, and lineage through the shared pass lifecycle; later passes allocate and compute only residual grid plus compensation. Reject a mode-specific alternate runner. |
