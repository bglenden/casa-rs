...
impl WorkImplementation for RecordingExecutor {
    type Error = io::Error;
    type ObservationReadCompletion = ();
...
fn plan_seals_physical_work_and_every_required_binding() {
    ...
    assert_eq!(
        ...
        [
            188, 248, 135, 221, 143, 91, 227, 244, 180, 184, 94, 29, 96, 96, 105, 119, 11, 183,
            107, 29, 253, 77, 121, 84, 77, 19, 183, 133, 81, 201, 208, 139,
        ]
    );
}
...
#[test]
fn t15_private_walking_skeleton_composes_landed_interfaces_and_failure_cuts() {
    ...
    .expect("T15 synthetic logical compilation");
    assert_eq!(
        problem.normal_equation().output().normalization(),
        casa_imaging_model::NormalStateNormalization::Unnormalized
    );
    assert_eq!(
        problem.product_graph().normalization_boundary(),
        problem.products().normalization_boundary()
    );
    assert_eq!(
        problem.product_graph().publication().members().len(),
        problem.products().products().len()
    );
    assert_eq!(
        problem
            .observation_transaction()
    ...
    .expect("T15 Resource Authority-backed physical planning");
    assert_eq!(execution_plan.problem_id(), problem.problem_id());
    assert_eq!(
        execution_plan.product_graph_id(),
        problem.product_graph().graph_id()
    ...
    assert_eq!(
        execution_plan.publication_layouts().entries().len(),
        problem.product_graph().publication().members().len() + 1,
    ...
    let success_directory = tempfile::tempdir().expect("success receipt directory");
    let success_receipts = ExecutionReceiptStore::new(
        success_directory.path(),
        ReceiptRetention::new(1, 1_048_576).expect("bounded retention"),
    )
    .expect("success receipt store");
    let success_provenance = execution_provenance(
        casa_imaging_runtime::ExecutionAttemptId::from_sha256([151; 32]),
        BuildIdentity::from_sha256([152; 32]),
    );
    let publication_launched = Arc::new(AtomicBool::new(false));
    let visible_generation = Arc::new(AtomicUsize::new(0));
    let success_registry = TestRegistry {
        id: registry(3),
        executors: BTreeMap::from([(
            implementation(6),
            publication_recording_executor(
    ...
    assert_eq!(
        run_receipted(
            ...
            authority(),
            &mut completion,
            success_receipts.bind(success_provenance.clone()),
        )
        .expect("T15 synthetic execution"),
        ExecutionOutcome::Succeeded
    );
    assert!(publication_launched.load(Ordering::SeqCst));
    assert_eq!(visible_generation.load(Ordering::SeqCst), 1);

    let success = success_receipts
        .open(success_provenance.attempt_id())
        .expect("reopen completed T15 receipt");
    assert_eq!(success.status(), ReceiptStatus::Completed);
    assert_eq!(success.problem_identity(), problem.problem_id().as_bytes());
    assert_eq!(
        success.product_graph_identity(),
        problem.product_graph().graph_id().as_bytes()
    );
    assert_eq!(success.plan_identity(), execution_plan.plan_id().as_bytes());
    assert_eq!(
        success.publication_layout_count(),
        execution_plan.publication_layouts().entries().len()
    );
    for layout in execution_plan.publication_layouts().entries() {
        let expected_participant = match layout.participant() {
        ...
        assert_eq!(
            success.publication_participant(layout.artifact()),
            Some(expected_participant)
        );
        assert_eq!(
            success.publication_layout_identity(layout.artifact()),
            Some(layout.layout_id())
        );
        assert_eq!(
            success.artifact_disposition(layout.artifact()),
            Some(ArtifactDisposition::Published)
        );
    }

    let cancelled_directory = tempfile::tempdir().expect("cancelled receipt directory");
    let cancelled_receipts = ExecutionReceiptStore::new(
        cancelled_directory.path(),
        ReceiptRetention::new(1, 1_048_576).expect("bounded retention"),
    )
    .expect("cancelled receipt store");
    let cancelled_provenance = execution_provenance(
        casa_imaging_runtime::ExecutionAttemptId::from_sha256([153; 32]),
        BuildIdentity::from_sha256([154; 32]),
    );
    let cancelled_visibility = Arc::new(AtomicUsize::new(0));
    let cancelled_registry = TestRegistry {
        id: registry(3),
        executors: BTreeMap::from([(
            implementation(6),
            publication_recording_executor(
                ...
                Arc::clone(&cancelled_visibility),
            ),
        )]),
    };
    ...
    assert_eq!(
        ...
        .expect("T15 cancellation drains"),
        ExecutionOutcome::Cancelled
    );
    let cancelled = cancelled_receipts
        .open(cancelled_provenance.attempt_id())
        .expect("reopen cancelled T15 receipt");
    assert_eq!(cancelled.status(), ReceiptStatus::Cancelled);
    assert_eq!(cancelled_visibility.load(Ordering::SeqCst), 0);
    assert_eq!(
        cancelled.node_status(&WorkNodeId::new("transaction-commit")),
        Some(ReceiptStatus::Cancelled)
    );

    let failed_directory = tempfile::tempdir().expect("failed receipt directory");
    let failed_receipts = ExecutionReceiptStore::new(
        failed_directory.path(),
        ReceiptRetention::new(1, 1_048_576).expect("bounded retention"),
    )
    .expect("failed receipt store");
    let failed_provenance = execution_provenance(
        casa_imaging_runtime::ExecutionAttemptId::from_sha256([155; 32]),
        BuildIdentity::from_sha256([156; 32]),
    );
    let failed_visibility = Arc::new(AtomicUsize::new(0));
    let failed_registry = TestRegistry {
        id: registry(3),
        executors: BTreeMap::from([(
            implementation(6),
            failing_transaction_executor(
                6,
                Arc::clone(&failed_visibility),
    ...
    .expect_err("T15 product staging failure is terminal");
    assert!(matches!(
        failure,
        RunError::Execution { ref node, .. } if node == &WorkNodeId::new("transaction-stage-psf")
    ));
    let failed = failed_receipts
        .open(failed_provenance.attempt_id())
        .expect("reopen failed T15 receipt");
    ...
    assert_eq!(failed_visibility.load(Ordering::SeqCst), 0);
    assert_eq!(
        failed.node_status(&WorkNodeId::new("transaction-commit")),
        Some(ReceiptStatus::Cancelled)
    );
}
