...
| Interleaved tiled-bucket reuse | casacore `TSMCube` caches a whole interleaved bucket by tile number, and `TSMDataColumn` copies each requested column section from that shared bucket (`TSMCube.cc:374-378`, `905-1005`; `TSMDataColumn.cc:205-288`). CASA and LibRA obtain their standard visibility cubes through that common table/visibility-buffer path. | The Rust typed selected-channel reader restricts access to intersecting channel tiles and uses 16 MiB sequential read-ahead, but creates a column-local tile session. On the measured one-channel row it returned 65,505,024 DATA bytes from 1,353,381,120 DATA tile bytes per pass; tile load, not copying, dominated. | `casa-tables` for physical bucket reads and accounting; `casa-ms` for the closed selected column set. | **Instrument before adapting.** Preserve exact physical-byte/read accounting and consider grouped interleaved-column fills only if a matched full-shape source stage remains material. The 64-channel scientific discriminator is compute-bound, so this is not the active optimization candidate. |
...
| Residual-only later major passes | CASA separates PSF construction (`dopsf=true`) from ordinary model-degrid/residual major cycles (`dopsf=false`) (`SynthesisImagerVi2.cc:1392-1428`, `1447-1475`, `1488-1489`). The last optimized pre-cutover implementation had dedicated bounded residual-refresh construction and accumulation (`fff9c2...:crates/casa-imaging/src/execution.rs:870-969`, `7332-7388`). | Candidate `77c5a6ccf` transplanted invariant normal-state reuse into reconstruction/runtime. On the pre-compact 64-channel baseline it reduced total serial wall by only 2.39% and final-major replay by 5.05%, despite bit-exact products, so `d49cca7d5` removed it. After the compact handoff changed the bottleneck, `5a3be7eeb` reduced final-major replay from 204.455013 to 183.448796 s (10.27%) and total wall from 487.939629 to 465.766714 s (4.54%). All 5,242,881 elements of all six products were bit-identical, with exact inventory and metadata. Production-shaped intermediate refreshes improved from about 179.8 to 159.6-160.0 s, but the full run still crossed CASA's 733.660 s wall during the third of ten later-major replays. | Reconstruction for scientific invariants; runtime for carried lifetime and residency. | **Adapted and retained.** Carry validated invariant normal state through the shared lifecycle with unchanged pass count, worker count, and bounded source residency. Treat it as one serial kernel improvement, not a substitute for removing the remaining scalar handoff cost. |
| Disk-backed gridded residual replay | The pre-cutover AW path sealed checksummed, source-major replay programs into unlinked temporary files and streamed them with bounded prefetch; mmap and partial-atlas variants were rejected after measured thrashing. CASA's `SIImageStore` uses disk-backed `TempImage<Complex>` forward/backward grids above a small memory allowance, while `GridFT` pages larger lattices through `LatticeCache`. LibRA can persist the device gridded state with `saveGriddedData`. | Current later passes rebuild spectral/weighting records and repeat degrid plus compensated grid for every selected correlation. Across six distributed real-shaped windows, the 263,250-row discriminator found 29,169,920 contributing scalar records but only 14,520,468 deterministically sorted block-local gridded tap records (2.0089x reduction); a constant-memory global estimate was 13,359,087 (2.1835x). The concrete 232,327,488-byte artifact was written in 29.24 ms, read sequentially in 100.90 ms, and verified by SHA-256. Streaming grouped application took 3.130 s between stable 7.566- and 7.589-second current-path baselines, a 58.70% point improvement. Linear extrapolation gives a 3,613,157,104-byte full artifact and a 48.67-second later replay, but neither is yet a full-MeasurementSet measurement. The unchanged replay retained normal-state identity `e636811...`, and the grouped normal-operator test agrees within `1e-15` normalized RMS. | Reconstruction defines and applies the gridded normal-operator record; runtime owns the run-scoped disk artifact, buffer budget, checksums, read-ahead, backpressure, and measurements. | **Apply gate passed; production seam requires explicit cutover approval.** Preserve deterministic bounded block reduction and one scientific path; retain the observed dirty grid and compile only model-dependent normal-operator records. Do not persist ungridded visibility records, expose a public cache format, mmap the artifact, or create an MFS-only application path. Evidence: `20260828-issue540-gridded-replay-cardinality.json`. |
