...
| Residual-only later major passes | CASA separates PSF construction (`dopsf=true`) from ordinary model-degrid/residual major cycles (`dopsf=false`) (`SynthesisImagerVi2.cc:1392-1428`, `1447-1475`, `1488-1489`). The last optimized pre-cutover implementation had dedicated bounded residual-refresh construction and accumulation (`fff9c2...:crates/casa-imaging/src/execution.rs:870-969`, `7332-7388`). | Candidate `77c5a6ccf` transplanted invariant normal-state reuse into reconstruction/runtime. On the pre-compact 64-channel baseline it reduced total serial wall by only 2.39% and final-major replay by 5.05%, despite bit-exact full-native products, so `d49cca7d5` removed it. The accepted compact handoff then reduced the whole discriminator by 16.59% and exposed compensated gridding as 23.7% of terminal-replay main-thread leaf samples, approximately 26 to 27% without terminal-only visibility hashing. The frozen full workload still crossed CASA wall after only two of ten later-major replays. | Reconstruction for scientific invariants; runtime for carried lifetime and residency. | **Re-evaluate on the changed measured bottleneck.** Retain only if the compact-baseline full-shape receipt materially improves and all six native products remain exact, with unchanged pass count, worker count, and bounded residency. |
