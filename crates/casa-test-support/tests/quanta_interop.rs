...
use casa_types::quanta::{MvAngle, MvTime, Unit, constants};
...
fn cross_validate_unit_factors() {
    ...
    for unit in &all_units {
        ...
        let cpp_factor = QuantaOracle::parse_factor(unit)
            .unwrap_or_else(|error| panic!("C++ parse failed for {unit}: {error}"));
...
fn cross_validate_conformance() {
    ...
    for (a, b) in &conformant_pairs {
        ...
        let cpp_conf = QuantaOracle::conformant(a, b).unwrap();
...
fn cross_validate_unit_dimensions() {
    ...
    for unit in &units {
        let rust = Unit::new(unit).unwrap_or_else(|e| panic!("Rust parse failed for {unit}: {e}"));
        let (_, cpp_dims) = QuantaOracle::parse_full(unit)
            .unwrap_or_else(|error| panic!("C++ parse_full failed for {unit}: {error}"));
...
fn cross_validate_qc_speed_of_light() {
    assert_close(
        constants::c().value(),
        QuantaOracle::speed_of_light().unwrap(),
        1e-15,
        "QC::c",
    );
}
...
fn cross_validate_qc_planck() {
    assert_close(
        constants::h().value(),
        QuantaOracle::planck_constant().unwrap(),
        1e-12,
        "QC::h",
    );
}
...
fn cross_validate_all_qc_constants() {
    ...
    let constants_table: Vec<(&str, &dyn Fn() -> &'static casa_types::quanta::Quantity)> = vec![
        ("c", &constants::c),
    ...
    for (cpp_name, rust_fn) in &constants_table {
        let rust_q = rust_fn();
        let (cpp_value, cpp_unit_str, cpp_dims) = QuantaOracle::constant(cpp_name)
            .unwrap_or_else(|error| panic!("C++ QC constant '{cpp_name}' failed: {error}"));
    ...
    let cases = [
        (
            MvAngle::from_radians(-0.25 * std::f64::consts::TAU)
                .normalized(0.0)
                .format_time(6),
            QuantaOracle::format_angle_time(-0.25 * std::f64::consts::TAU, 0.0, 6).unwrap(),
            "ra_time_wrap",
        ),
        (
            MvAngle::from_radians((-12.5_f64).to_radians()).format_angle_dig2(5),
            QuantaOracle::format_angle_dig2((-12.5_f64).to_radians(), 5).unwrap(),
            "declination_dig2",
        ),
        (
            MvAngle::from_radians(123.5_f64.to_radians()).format_angle(1),
            QuantaOracle::format_angle(123.5_f64.to_radians(), 1).unwrap(),
            "longitude_angle",
        ),
    ];
...
fn cross_validate_mvtime_formatting_subset() {
    ...
    let cases = [
        (
            ...
            QuantaOracle::format_mjd_dmy(j2000, 1).unwrap(),
            "j2000_dmy",
        ),
        (
            MvTime::from_mjd_days(j2000).format_time(1),
            QuantaOracle::format_mjd_time(j2000, 1).unwrap(),
            "j2000_time",
        ),
        (
            MvTime::from_mjd_days(j2000).format_dmy_date(),
            QuantaOracle::format_mjd_date(j2000).unwrap(),
            "j2000_date",
        ),
        (
            MvTime::from_mjd_days(rounded_boundary).format_dmy(1),
            QuantaOracle::format_mjd_dmy(rounded_boundary, 1).unwrap(),
            "round_day_boundary",
        ),
    ];
