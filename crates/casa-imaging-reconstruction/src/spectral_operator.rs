...
pub struct SpectralOperatorSpecification {
    ...
    direction_independent_polarizations: Box<[PolarizationOperator]>,
    image_shape: [usize; 2],
    grid_shape: [usize; 2],
    image_blc: [usize; 2],
    increment_rad: [f64; 2],
...
mod specification_metadata_tests {
    ...
    fn owned_metadata_counts_string_capacity_and_excludes_shared_catalog_payload() {
        ...
        assert_eq!(specification.direction_independent_polarizations.len(), 1);
        let correlations = specification.direction_independent_polarizations[0]
            .correlations()
            .to_vec();
        let compiled = specification
            .direction_independent_polarization(&correlations)
            .expect("selected layout is compiled");
        let reference = PolarizationOperator::compile(
        ...
        assert!(
            specification
                .direction_independent_polarization(&[])
                .is_err()
        );
        assert!(
            specification
        ...
        let matrices = specification.direction_independent_polarizations.clone();
        specification.direction_independent_polarizations = Box::default();
        assert_eq!(
            initial - specification.owned_heap_bytes().unwrap(),
            std::mem::size_of_val(matrices.as_ref()),
            "precompiled matrix payload remains in the physical demand",
        );
        specification.direction_independent_polarizations = matrices;
...
impl SpectralOperatorSpecification {
    pub(crate) fn with_prediction_range(
        mut self,
        resident: std::ops::Range<usize>,
    ) -> Result<Self, SpectralOperatorError> {
        if self.basis != SpectralBasisPlan::ChannelLocal
            || resident.start > self.slab.core_start
            || resident.end < self.slab.core_end
            || resident.end > self.slab.total_channels
        {
            return Err(SpectralOperatorError::InvalidSlab);
        }
        self.slab.resident_start = resident.start;
        self.slab.resident_end = resident.end;
        Ok(self)
    }
    ...
    pub fn for_slab(
        ...
        let mut direction_independent_polarizations = Vec::<PolarizationOperator>::new();
        for selection in problem
            .inputs()
            ...
            .flat_map(|source| source.selection().correlations())
        {
            ...
            if !direction_independent_polarizations
                .iter()
                .any(|operator| operator.correlations() == correlations.as_slice())
            {
                direction_independent_polarizations.push(
        ...
        Ok(Self {
            ...
            direction_independent_polarizations: direction_independent_polarizations
                .into_boxed_slice(),
    ...
    /// Reuse the sky-frame map compiled for this exact selected correlation layout.
    /// Calibrated standard-imaging correlations are already in the sky frame;
    /// parallactic angles and Mueller responses remain in their paired
    /// direction-dependent operator, not this direction-independent map.
    pub(super) fn direction_independent_polarization(
        ...
        self.direction_independent_polarizations
            .iter()
            .find(|operator| operator.correlations() == correlations)
            .ok_or(SpectralOperatorError::InvalidSample)
    }
...
pub struct PreparedSpectralOperatorRecycle {
    pub(crate) ffts: Vec<PreparedFft>,
...
impl PreparedSpectralOperator {
    /// Return the admitted core and complete prediction range of this prepared window.
    pub fn slab(&self) -> SpectralSlabPlan {
...
impl ReusableNormalState {
    ...
    fn matches(
        ...
        self.problem == specification.problem
            ...
            && self.slab.total_channels() == specification.slab.total_channels()
...
fn combine_initial_chart_primitives(
    ...
    for domain in specification.domains() {
        ...
        let parent_cells = checked_cells(parent_shape)?;
        let first = chart_primitives
            .next()
            .transpose()?
            .ok_or(SpectralOperatorError::DomainProjectionMismatch)?;
        if matches!(
            &specification.charts[domain.chart_range()],
            [chart] if chart.window.origin() == [0, 0]
                && chart.geometry.image_shape == parent_shape
        ) {
            if first.shape != parent_shape {
                return Err(SpectralOperatorError::DomainProjectionMismatch);
            }
...
/// Channel-local slabs write disjoint output windows into their admitted backing.
/// The opaque owner verifies coverage without making a complete-axis image allocation.
#[doc(hidden)]
#[derive(Debug)]
...
enum CompleteDataOwnerSlabFoldState {
    Coupled(CompleteDataOwnerResult),
    ChannelLocal(normal_storage::StoredChannelNormalFold),
}

impl CompleteDataOwnerSlabFold {
    /// Start an ordered fold using an explicitly admitted physical storage plan.
    pub fn begin(
        first: CompleteDataOwnerResult,
        storage: &normal_storage::NormalStoragePlan,
    ) -> Result<Self, SpectralOperatorError> {
        ...
        let state = match first.primitives().basis {
            SpectralBasisPlan::TaylorViaChannelMajor(_) => {
                CompleteDataOwnerSlabFoldState::Coupled(first)
            }
    ...
    pub fn extend(self, next: CompleteDataOwnerResult) -> Result<Self, SpectralOperatorError> {
        let state = match self.state {
            CompleteDataOwnerSlabFoldState::Coupled(prefix) => {
                CompleteDataOwnerSlabFoldState::Coupled(
    ...
    pub fn finish(self) -> Result<normal_storage::CompleteDataNormalState, SpectralOperatorError> {
        match self.state {
            CompleteDataOwnerSlabFoldState::Coupled(result) => {
                let slab = result.primitives().slab();
                if slab.core_range() != (0..slab.total_channels()) {
                    return Err(SpectralOperatorError::IncompleteCoverage);
                }
                Ok(normal_storage::CompleteDataNormalState {
                    primitives: normal_storage::NormalStatePrimitives::Coupled(result.domains),
...
impl SpectralScienceProbe {
    ...
    fn from_environment(
        ...
        let probe = Self::configured(imaging_science_trace_enabled(), || {
            ...
            > = OnceLock::new();
            CONFIG
                .get_or_init(|| match std::env::var("CASA_RS_TRACE_AW_GROUP") {
...
impl CompleteDataOwnerState {
    ...
    pub fn predict_block(
        ...
        for group in block.correlation_groups() {
            ...
            let polarization = self.specification.direction_independent_polarization(
                &group
                    .iter()
                    .map(|weighted| weighted.selected().address.correlation_type)
                    .collect::<SmallVec<[_; 4]>>(),
...
impl SpectralSlabOperator {
    pub(crate) fn owns_gridded_plane(&self, output_plane: usize) -> bool {
        self.basis != SpectralBasisPlan::ChannelLocal
            || self.slab.owns(output_plane / self.polarization_count)
    }
    ...
    fn validate_model_generation(
        ...
        if shape.domains().len()
            ...
            || generation.sample_count() != shape.sample_count()
        {
    ...
    fn prepare_forward_generation(
        ...
        for (resident, coefficient) in coefficient_range.enumerate() {
            let generation =
                &generation.read_window(self.domain_ordinal, coefficient..coefficient + 1)?;
            for polarization in 0..self.polarization_count {
    ...
    fn prepare_primary_beam_forward_generation(
        ...
        let aw_projection = self.aw_projection.is_some();
        let generation =
            &generation.read_window(self.domain_ordinal, 0..plan.coefficient_term_count())?;
    ...
    pub(crate) fn predict_gridded_normal_aw_polarization(
        ...
        let mut prediction_coordinates = coordinates;
        if coordinates.uvw_m[2] <= 0.0 {
            prediction_coordinates.mueller_element = match coordinates.mueller_element {
                0 => 15,
                15 => 0,
                _ => return Err(SpectralOperatorError::GriddedRecordMismatch),
            };
        }
        ...
        let predicted = match self.basis {
            ...
            SpectralBasisPlan::Polynomial(plan) => {
                let coefficients = plan
                    .weighted_coefficients(coordinates.frequency_hz, 1.0)
                    .map_err(|_| SpectralOperatorError::GriddedRecordMismatch)?;
    ...
    pub(crate) fn grid_gridded_normal_local_aw_polarization(
        ...
        let shape = [grids[0].nrows(), grids[0].ncols()];
        let plan = self
            .aw_projection
            .as_ref()
            .ok_or(SpectralOperatorError::GriddedRecordMismatch)?
        ...
        let mut grid_term = |term: usize, coefficient: f64| -> Result<(), SpectralOperatorError> {
            let plane = self.polarization_plane(term, polarization);
            let gridded = predicted * (adjoint_scale * coefficient);
            if !gridded.re.is_finite() || !gridded.im.is_finite() {
                return Err(SpectralOperatorError::GeneratedNonfinite);
            }
        ...
        match self.basis {
            SpectralBasisPlan::Polynomial(basis) => {
                for (term, coefficient) in basis
                    .weighted_coefficients(coordinates.frequency_hz, imaging_weight)
                    .map_err(|_| SpectralOperatorError::GriddedRecordMismatch)?
                    .enumerate()
                {
                    grid_term(
                        term,
                        coefficient.map_err(|_| SpectralOperatorError::GriddedRecordMismatch)?,
                    )?;
                }
                Ok(())
            }
...
pub enum SpectralOperatorError {
    ...
    /// Gridded replay cannot represent the compiled basis and kernel combination.
    #[error(
        "gridded normal-operator replay does not support the compiled basis/kernel combination"
    )]
...
mod tests {
    ...
    struct ObserverAwProvider {
        cells: Vec<(PreparedArtifactScientificIdentity, Arc<AwConvolutionCell>)>,
    }
    ...
    impl AwPreparedCellProvider for ObserverAwProvider {
        fn load(
            ...
            let cell = &self
                .cells
                .iter()
                .find(|(identity, _)| *identity == metadata.identity())
                .ok_or(AwOperatorError::InvalidCatalogLayout)?
                .1;
    ...
    #[test]
    fn t51_direct_taylor_aw_replay_matches_source_action_with_signed_weights_and_hands() {
        let layout = AwKernelLayout::new([0, 0], 1, [3, 3], [1, 1]).unwrap();
        let mut metadata = Vec::new();
        let mut cells = Vec::new();
        for (mueller, kernel) in [
            (0, Complex64::new(3.0, 4.0)),
            (15, Complex64::new(7.0, -2.0)),
        ] {
            let identity = PreparedArtifactScientificIdentity::convolution_function(
                PreparedArtifactCellSemantics::new(
                    1.0e9,
            ...
            metadata.push(
                AwPreparedCellMetadata::new(
                    identity, 1.0e9, 1.0, 1.0, mueller, 0.0, layout, layout,
                )
                .unwrap(),
            );
            let mut values = vec![Complex64::default(); 9];
            values[4] = kernel;
            let convolution = AwConvolutionKernel::new(layout, values).unwrap();
            cells.push((
                identity,
                Arc::new(
                    AwConvolutionCell::new(identity, convolution.clone(), convolution).unwrap(),
                ),
            ));
        }
        let prepared = crate::PreparedAwProjection::new(
            AwPreparedCatalog::new(metadata).unwrap(),
            ObserverAwProvider { cells },
            false,
            64 * 1024,
        )
        .unwrap();
        let workload = workload();
        let mut source = SpectralSlabOperator::new_inner(
            super::SpectralSlabDefinition {
                specification: None,
                chart_ordinal: 0,
                domain_ordinal: 0,
                facet_ordinal: 0,
        ...
        .unwrap();
        for (term, grid) in source.forward_grids.iter_mut().enumerate() {
            for ((x, y), value) in grid.indexed_iter_mut() {
                *value = Complex64::new(
                    (x + 3 * y + 5 * term) as f64 / 31.0,
                    (2 * x + y + term) as f64 / 23.0,
                );
            }
        }
        ...
        for frequency_hz in [0.91e9, 1.19e9] {
            for w_m in [2.0, -2.0] {
                for (hand, mueller) in [0, 15].into_iter().enumerate() {
                    let weight = if hand == 0 { 0.1 } else { 0.31 };
                    let sample = SpectralOperatorSample::new(
                        0,
                        [0.25, -0.4, w_m],
                        frequency_hz,
                    ...
                    .unwrap()
                    .with_aw_coordinates([0.0; 2], [0.0; 2], mueller, -1.5 * w_m)
                    .unwrap();
                    let selected_hand =
                        super::aw_gridding_input_index(&correlations, hand, w_m).unwrap();
                    let mut input_sample = sample;
                    input_sample.mueller_element = [0, 15][selected_hand];
                    let source_prediction =
                        source.predict_one_polarization(input_sample, 0).unwrap();
                    let coordinates = super::AwReplayCoordinates::from_sample(sample).unwrap();
                    let forward_scale = sample.phase().conj();
                    let replay_prediction = source
                        .predict_gridded_normal_aw_polarization(0, 0, coordinates, forward_scale)
                        .unwrap();
                    assert!(
                        (source_prediction - replay_prediction).norm()
                            <= 1.0e-6 * source_prediction.norm().max(1.0)
                    );
                    let mut gridding_sample = sample;
                    gridding_sample.visibility = source_prediction;
                    for _ in 0..3 {
                        source.push(gridding_sample).unwrap();
                    }
                    let psf_before = source.psf_grids.clone();
                    source
                        ...
                        .unwrap();
                    assert_eq!(
                        source.psf_grids, psf_before,
                        "replay cannot mutate frozen normal moments"
                    );
                }
            }
        }
        let source_grids = source.dirty_grids.unwrap();
        let squared_error: f64 = source_grids
            .iter()
            .flatten()
            .zip(replay_grids.iter().flatten())
            .map(|(left, right)| (*left - *right).norm_sqr())
            .sum();
        let squared_signal: f64 = source_grids.iter().flatten().map(Complex64::norm_sqr).sum();
        let nrms = (squared_error / squared_signal).sqrt();
        eprintln!("t51_direct_taylor_aw_source_replay_nrms={nrms:.17e}");
        assert!(nrms <= 0.001, "coordinate-backed Taylor replay NRMS {nrms}");
    }
