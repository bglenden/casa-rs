...
/// Map a `ManagerKind` to `casa_tables::DataManagerKind`.
fn to_dm_kind(manager: ManagerKind) -> casa_tables::DataManagerKind {
    match manager {
        ManagerKind::StManAipsIO => casa_tables::DataManagerKind::StManAipsIO,
        ManagerKind::StandardStMan => casa_tables::DataManagerKind::StandardStMan,
        ManagerKind::IncrementalStMan => casa_tables::DataManagerKind::IncrementalStMan,
        ManagerKind::TiledColumnStMan => casa_tables::DataManagerKind::TiledColumnStMan,
        ManagerKind::TiledShapeStMan => casa_tables::DataManagerKind::TiledShapeStMan,
        ManagerKind::TiledCellStMan => casa_tables::DataManagerKind::TiledCellStMan,
    }
}
...
fn build_table_from_fixture(fixture: &TableFixture) -> Result<Table, TableError> {
    ...
    for field in fixture.table_keywords.fields() {
        table.keywords_mut().push(casa_types::RecordField::new(
            field.name.clone(),
            field.value.clone(),
        ));
    }
...
mod tests {
    use super::*;
    use casa_types::{PrimitiveType, RecordField, ScalarValue};
    ...
    fn compare_table_to_fixture_reports_mismatches() {
        ...
        schema_mismatch.schema = TableSchema::new(vec![casa_tables::ColumnSchema::scalar(
            "id",
            PrimitiveType::Int32,
        )])
        .unwrap();
