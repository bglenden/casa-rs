...
fn log_imaging_stage_timing(
    ...
    if let Some(started) = started {
        eprintln!(
            "imaging_science_stage_timing stage={stage} pass_phase={:?} pass_ordinal={} elapsed_nanos={}",
            pass.phase(),
            pass.ordinal(),
...
struct SpectralCycleExecutorState {
    ...
    gridded_admission: Option<crate::complete_data_operator::GriddedNormalCompilationAdmission>,
...
impl SpectralCycleExecutor {
    ...
    pub fn with_planned_gridded_normal_binding(
        ...
        let (storage, maximum_bytes, admission) =
        ...
        state.gridded_admission = Some(admission);
        Ok(self)
    }
    ...
    fn run_stream(
            ...
            && let Some(storage) = state.gridded_storage.as_ref()
        {
            state.gridded_compilation = Some(GriddedNormalReplayCompilation::new(
                ...
                state
                    .gridded_admission
                    .ok_or_else(|| io::Error::other("missing admitted gridded compiler layout"))?,
            )?);
        }
        ...
        *operator = Some(first_operator);
             ...
             execution: crate::bounded_stream::BoundedExecution<'_>| {
                ...
                let mut science = || {
                    let predicted = operator
                        ...
                        .consume_bounded_replay_chunk(block, execution)
                        .map_err(io::Error::other)?;
        ...
        for ordinal in 1..self.complete_data.slab_count() {
            ...
            next_operator
                ...
                .map_err(io::Error::other)?;
            let source_window_hz = next_operator.initial_source_window_hz(&self.problem);
            if let Some(bounds) = source_window_hz {
                next_operator
                    .authorize_initial_source_window(&self.problem, bounds)
                    .map_err(io::Error::other)?;
            }
    ...
    fn log_gridded_replay_measurements(&self, replay: &FrozenGriddedNormalReplay) {
        ...
        let core_window = window
            .selected_window((replay.artifact_pass_count() - 1) as usize)
            .map_or(window, |(_, core)| core);
        let planned_windows: usize = core_window
            .frame_counts()
            .iter()
            .map(|count| count.div_ceil(stream.maximum_logical_units_per_block))
            .sum();
        eprintln!(
            ...
            self.complete_data
                ...
                .map(|residency| residency.maximum_window_records())
                .unwrap_or(0),
            self.complete_data
                .gridded_route_residency()
            ...
            self.complete_data
                .gridded_route_residency()
                .map(|residency| residency.maximum_frames())
                .unwrap_or(0),
            self.complete_data.residency().gridded_route_bytes(),
            routing.frames_routed,
            routing.encoded_records,
            routing.routed_record_memberships,
            routing.prediction_groups,
            routing.degrid_records,
            routing.grid_records,
            routing.sector_rescans,
            routing.peak_physical_route_capacity_bytes,
        ...
        let result = (|| -> Result<WorkMeasurements, io::Error> {
                ...
                && self.mode == SpectralCycleExecutionMode::Science
            {
                ...
                let result = owner
                    ...
                    .map_err(io::Error::other)?;
                log_imaging_stage_timing("major_cycle_reconciliation", self.pass, started);
...
impl ReconstructionCyclePhaseCompletion {
    ...
    /// Return the exact support generation set used for component placement.
    #[must_use]
    pub const fn masks(&self) -> &ReconstructionMaskSet {
        &self.masks
    }
