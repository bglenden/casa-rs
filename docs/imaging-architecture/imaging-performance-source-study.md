...
| Residual-only later major passes | CASA separates PSF construction (`dopsf=true`) from ordinary model-degrid/residual major cycles (`dopsf=false`) (`SynthesisImagerVi2.cc:1392-1428`, `1447-1475`, `1488-1489`). The last optimized pre-cutover implementation had dedicated bounded residual-refresh construction and accumulation (`fff9c2...:crates/casa-imaging/src/execution.rs:870-969`, `7332-7388`). | Candidate `70d823603` was bit-exact but its one-channel wall was unchanged. Later tile profiling showed that row had 20.66-fold DATA storage amplification and hid channel-scaled gridding work. The complete 64-channel parent instead spent 250.632 of 250.704 seconds in the later-major kernel versus 10.484 seconds reading source data. | Reconstruction for scientific invariants; runtime for carried lifetime and residency. | **Adapt and judge on the complete channel shape.** Carry validated invariant normal state through the shared lifecycle and require exact products plus a material reduction of the full-shape later-major receipt; do not restore an alternate runner. |
...
| Row/channel run payloads instead of scalar inbox items | Historical tests found row/channel runs removed scalar synchronization; scalar direct inbox took 258.34 s wall, 1020.53 s system, and 14.4 M drains (`imperformance-wave-2-experiment-summary.md:138-143`, `155-156`). | `casa-ms` buffers rows internally but exports owned scalar samples. No current copy/scalarization cost measurement exists. | `casa-ms` owns physical block view; reconstruction owns scientific run representation; runtime transports leases. | **Measure, then adapt if material.** Do not introduce an unproven conversion layer. |
