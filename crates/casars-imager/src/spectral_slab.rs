...
impl VisibilitySourceShape {
    pub(crate) fn row_channel_bytes(&self) -> usize {
        self.corr_count
    ...
    pub(crate) fn row_fixed_physical_read_bytes(&self) -> usize {
        self.corr_count
    ...
    pub(crate) fn row_fixed_resident_bytes(&self) -> usize {
        self.resident_layout.bytes_per_row()
    }

    pub(crate) fn row_cache_overhead_bytes(&self) -> usize {
    ...
    pub(crate) fn source_read_bytes(
        &self,
        logical_channel_visits: usize,
        fixed_row_reads: usize,
    ) -> usize {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct SpectralCandidateModel {
    ...
    pub(crate) worker_model: &'static str,
    pub(crate) modeled_worker_runtime_cost_units: u128,
    pub(crate) backend: &'static str,
...
impl SpectralCandidateModel {
    pub(crate) fn log_line(&self) -> String {
        format!(
            ...
            self.worker_count,
            self.worker_model,
            self.modeled_worker_runtime_cost_units,
            self.backend,
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct SpectralCandidateInput {
    ...
    pub(crate) worker_work_units_per_plane: usize,
    pub(crate) requirements: PlaneStateRequirements,
    pub(crate) prepared_residency: PreparedVisibilityResidency,
}
...
struct CandidatePlan {
    ...
    modeled_worker_runtime_cost_units: u128,
...
pub(crate) fn model_spectral_candidates(
    input: SpectralCandidateInput,
) -> Result<SpectralCandidateModel, String> {
    ...
    Ok(SpectralCandidateModel {
        ...
        worker_model: "work_units_per_worker_wave",
...
fn build_streaming_candidate(
    ...
    let modeled_worker_runtime_cost = (input.worker_work_units_per_plane as u128).saturating_mul(
        total_plane_worker_waves(input.output.plane_count, shape.active_planes, worker_count)
            as u128,
    );
    Some(CandidatePlan {
        schedule_kind,
        ...
        modeled_worker_runtime_cost_units: modeled_worker_runtime_cost,
        modeled_runtime_cost_bytes: modeled_runtime_cost_bytes(
            modeled_total_io_bytes,
            modeled_worker_runtime_cost,
            per_plane_state_bytes,
            input
...
fn modeled_runtime_cost_bytes(
    ...
    let modeled_worker_runtime_cost_bytes =
        usize::try_from(modeled_worker_runtime_cost_units).unwrap_or(usize::MAX);
    modeled_total_io_bytes
        .saturating_add(modeled_worker_runtime_cost_bytes)
        .saturating_add(
...
fn candidate_io_cost_fragment(candidate: CandidatePlan, executable: bool) -> String {
    ...
    let spill_io = candidate
        ...
        .saturating_add(candidate.modeled_output_spill_write_bytes);
    format!(
        "{}:runtime={},worker_runtime={},total={},source={},cache={},spill={},product={},product_groups={},active_planes={},slab_count={},row_block_rows={},worker_count={},scratch={},product_batch_planes={},cache_policy={},residency={},executable={}",
        candidate.schedule_kind.as_str(),
        candidate.modeled_runtime_cost_bytes,
        candidate.modeled_worker_runtime_cost_units,
        candidate.modeled_total_io_bytes,
        candidate.modeled_source_read_bytes,
        cache_io,
        spill_io,
...
mod tests {
    ...
    fn memory_planner_fits_minimum_planes_and_logs_breakdown() {
        ...
        let plan =
            model_spectral_candidates(planner_input(10, [512, 512], visibility, 512 * 1024 * 1024))
                .unwrap();
        ...
        assert!(plan.log_line().contains("worker_model="));
        assert!(
            plan.log_line()
                .contains("modeled_worker_runtime_cost_units=")
        );
    ...
    fn memory_planner_prefers_source_first_when_output_state_fits() {
        ...
        let plan =
            model_spectral_candidates(planner_input(8, [128, 128], visibility, 512 * 1024 * 1024))
                .unwrap();
    ...
    fn memory_planner_uses_worker_capacity_for_resident_planes() {
        ...
        let plan = model_spectral_candidates(input).unwrap();
    ...
    fn memory_planner_uses_slab_first_only_when_spill_exceeds_rereads() {
        let visibility =
            test_visibility_shape(64, 8, 1, 8, false, reread_all_source_slab_shapes(8, 8));
        let plan =
            model_spectral_candidates(planner_input(8, [512, 512], visibility, 10 * 1024 * 1024))
                .unwrap();
    ...
    fn memory_planner_does_not_cap_large_image_slabs_by_image_size_alone() {
        ...
        let plan = model_spectral_candidates(planner_input(
            512,
            [2048, 2048],
            visibility,
            16 * 1024 * 1024 * 1024,
    ...
    fn candidate_tie_break_prefers_fewer_source_blocks() {
        ...
        let base_candidate = CandidatePlan {
            ...
            modeled_worker_runtime_cost_units: 0,
    ...
    fn memory_planner_uses_actual_visibility_data_element_widths() {
        ...
        let plan32 = model_spectral_candidates(planner_input(
            8,
            [256, 256],
            complex32,
            4 * 1024 * 1024 * 1024,
        ...
        .unwrap();
        let plan64 = model_spectral_candidates(planner_input(
            8,
            [256, 256],
            complex64,
            4 * 1024 * 1024 * 1024,
    ...
    fn memory_planner_rejects_too_small_memory_target() {
        ...
        let error = model_spectral_candidates(input).unwrap_err();
