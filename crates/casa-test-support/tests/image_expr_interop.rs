...
fn make_image(
    ...
    shape: &[usize],
    values: Vec<f32>,
) -> Result<PagedImage<f32>, casa_images::ImageError> {
...
fn two_image_imgexpr_cross_matrix_matches_expected_pixels() {
    ...
    let rr = casa_images::expr_file::open::<f32>(&rust_expr_path)
        .unwrap()
        .get()
        .unwrap();
...
fn open_nonexistent_imgexpr_errors() {
    let result = casa_images::expr_file::open::<f32>("/nonexistent/path.imgexpr");
...
fn open_imgexpr_with_missing_source_errors() {
    ...
    casa_images::expr_file::save(
        &expr_path,
        "'nonexistent.image' + 1.0",
        casa_types::PrimitiveType::Float32,
        &casa_types::RecordValue::default(),
    )
    .unwrap();

    let result = casa_images::expr_file::open::<f32>(&expr_path);
    assert!(result.is_err());
}
