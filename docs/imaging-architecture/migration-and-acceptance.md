...
Under [ADR-0014](../adr/0014-trusted-product-generation-without-content-attestation.md),
trusted product generation transfers bounded owned windows directly to the
CASA-compatible writer's private staging. Publication checks exact graph
inventory, shape, source/run association, complete writes and lifecycle, not
content hashes. Seals, content commitments, completion authorities, projections
and verification-only full-array passes are not acceptance requirements.
The former T13/T22 two-phase attestation requirement is superseded and
non-normative. Scientific sources and cross-generation dependencies still must
match, and individual images are atomically replaced. Publication failure marks
the run failed and its output set incomplete; rerun is required, with no
per-member resumable recovery or whole-set rollback guarantee.
...
the product-owned PB tt0 generated by an explicit analytic telescope model
selected from observation metadata. Missing or unsupported telescope models
fail closed; broader response and mosaic PB correction remain with T47/#533.
T13/#499 lands the Product Graph and atomic store, T22/#508 owns the first
