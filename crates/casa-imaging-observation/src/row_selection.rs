...
use casa_ms::MainRowSelectionFact;
use thiserror::Error;
...
pub(crate) struct CompiledRowPredicate<'a> {
    selection: &'a RowSelection,
    data_descriptions: &'a [DataDescriptionSelection],
    wavelengths_by_ddid: Vec<(u32, f64)>,
}

impl<'a> CompiledRowPredicate<'a> {
    pub(crate) fn new(
        selection: &'a RowSelection,
        data_descriptions: &'a [DataDescriptionSelection],
        ...
        if matches!(selection.uv_distances(), UvSelection::Ranges(ranges) if ranges.iter().any(|range| range.unit() == UvDistanceUnit::Wavelengths))
        {
