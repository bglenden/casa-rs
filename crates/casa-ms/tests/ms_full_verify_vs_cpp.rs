...
fn schema_true_type_tag(col: &casa_tables::ColumnSchema) -> String {
...
fn schema_options_bits(col: &casa_tables::ColumnSchema) -> u32 {
    ...
    if matches!(
        col.column_type(),
        ColumnType::Array(casa_tables::ArrayShapeContract::Fixed { .. })
    ) {
        bits |= 4;
    }
    bits
...
fn schema_ndim_and_shape(col: &casa_tables::ColumnSchema) -> (i64, Vec<usize>) {
    match col.column_type() {
        ColumnType::Scalar | ColumnType::Record => (0, Vec::new()),
        ColumnType::Array(casa_tables::ArrayShapeContract::Fixed { shape }) => {
            (shape.len() as i64, shape.clone())
        }
        ColumnType::Array(casa_tables::ArrayShapeContract::Variable { ndim }) => {
    ...
    for name in column_names {
        ...
        let value = if let Some(value) = field {
            ...
            if matches!(
                schema
                    .column(&name)
                    .map(casa_tables::ColumnSchema::column_type),
...
fn digest_table_recursive(
    ...
    for ref_path in discovered_refs {
        ...
        let ref_table =
            Table::open(casa_tables::TableOptions::new(&ref_path)).expect("open referenced table");
...
///
/// `CASA_RS_VERIFY_MS_PATH=/path/to/uid___A002_Xb9dfa4_X4724_target_spw16.ms cargo test --release -p casa-ms --test ms_full_verify_vs_cpp bench_external_main_rows_vs_cpp -- --ignored --nocapture`
#[test]
...
/// Set `CASA_RS_VERIFY_MS_PATH` to the external MS root and run, for example:
///
/// `CASA_RS_VERIFY_MS_PATH=/path/to/uid___A002_Xb9dfa4_X4724_target_spw16.ms cargo test --release -p casa-ms --test ms_full_verify_vs_cpp bench_external_open_and_main_rows_vs_cpp_coldish -- --ignored --nocapture`
#[test]
