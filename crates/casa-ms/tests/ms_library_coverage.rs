...
use casa_ms::columns::data_columns::FloatDataColumn;
use casa_ms::columns::direction_columns::DirectionColumn;
use casa_ms::columns::exposure_interval::{ExposureColumn, IntervalColumn};
use casa_ms::columns::frequency_columns::ChanFreqColumn;
use casa_ms::columns::main_ids;
use casa_ms::columns::position_columns::AntennaPositionColumn;
...
use casa_types::{ArrayD, ArrayValue, PrimitiveType, RecordField, RecordValue, ScalarValue, Value};
...
fn generic_subtable_wrappers_cover_common_read_write_paths() {
...

    {
        ...
        assert_eq!(
            <casa_ms::MsObservation<'_> as SubTable>::id(),
...

    {
        ...
        assert!(
            <casa_ms::MsDataDescription<'_> as SubTable>::required_columns()
                .iter()
...
fn wrapper_mutators_and_generic_type_mismatch_paths_are_exercised() {
    let observation_schema =
        build_table_schema(casa_ms::schema::observation::REQUIRED_COLUMNS).unwrap();
    ...
    observation_table
        ...
        .unwrap();
    let observation = casa_ms::MsObservation::new(&observation_table);
    assert!(matches!(
    ...
    let field_schema = build_table_schema(casa_ms::schema::field::REQUIRED_COLUMNS).unwrap();
    ...
    field_table
        ...
        .unwrap();
    {
        let mut field = casa_ms::MsFieldMut::new(&mut field_table);
    ...
    let pol_schema = build_table_schema(casa_ms::schema::polarization::REQUIRED_COLUMNS).unwrap();
    ...
    pol_table
        ...
        .unwrap();
    {
        let mut pol = casa_ms::MsPolarizationMut::new(&mut pol_table);
    ...
    spw_table
        ...
        .unwrap();
    {
        let mut spw = casa_ms::MsSpectralWindowMut::new(&mut spw_table);
    ...
    let antenna_schema = build_table_schema(casa_ms::schema::antenna::REQUIRED_COLUMNS).unwrap();
    let mut antenna_table = Table::with_schema(antenna_schema);
    {
        let mut antenna = casa_ms::MsAntennaMut::new(&mut antenna_table);
        antenna
            .add_antenna(
                "ANT1",
                "PAD1",
    ...
    let antenna = casa_ms::MsAntenna::new(&antenna_table);
    ...
    let antenna_defs: Vec<ColumnDef> = casa_ms::schema::antenna::REQUIRED_COLUMNS
        .iter()
        .chain(casa_ms::schema::antenna::OPTIONAL_COLUMNS.iter())
        .copied()
        .collect();
    ...
    optional_antenna_table
        ...
        .unwrap();
    let optional_antenna = casa_ms::MsAntenna::new(&optional_antenna_table);
    assert_eq!(
...
fn validation_and_measure_columns_cover_error_paths() {
    ...
    assert!(issues.iter().any(|issue| matches!(
        issue,
        casa_ms::validate::ValidationIssue::WrongQuantumUnits { column_name, .. }
            if column_name == "TIME"
    )));
    assert!(issues.iter().any(|issue| matches!(
        issue,
        casa_ms::validate::ValidationIssue::WrongMeasureType { column_name, .. }
            if column_name == "TIME"
    )));
    assert!(
        issues
