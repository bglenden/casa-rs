...
fn prepared_physical_work(
    ...
    let dag = ExecutionDag::new(ExecutionDagSpecification {
        ...
        initial_knobs: ExecutionKnobs {
            ...
            ..base.execution_dag().initial_knobs().clone()
        },
        adaptations: base
            .execution_dag()
            .adaptations()
...
fn public_prepared_reuse_receipts_fail_closed_rejections() {
    ...
    .expect("missing reuse execution");
    assert_eq!(
    ...
    .expect("corrupt reuse remains a receipted rejection");
    assert_eq!(
    ...
    for (index, problem) in problems.iter().enumerate() {
        ...
        assert!(matches!(
            execute_prepared_operation(
                ...
                descriptor,
                PreparedArtifactOperation::Generate,
                121 + u8::try_from(index * 2).expect("small attempt index"),
                122 + u8::try_from(index * 2).expect("small build index"),
            ),
            PreparedObserved::Materialized { .. }
        ));
    }
