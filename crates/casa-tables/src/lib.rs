...
//! as [`casa_types::RecordValue`] maps. From C++ casacore's `Tables.h`:
//!
...
//!
//! ```rust
//! # use casa_tables::{Table, TableSchema, ColumnSchema};
//! # use casa_types::*;
...
//!
//! ```rust
//! # use casa_tables::{Table, TableSchema, ColumnSchema, SortOrder};
//! # use casa_types::*;
//! # let schema = TableSchema::new(vec![
//! #     ColumnSchema::scalar("id", PrimitiveType::Int32),
//! # ]).unwrap();
...
//!
//! ```rust,no_run
//! # use casa_tables::{Table, TableOptions, DataManagerKind};
...
//!
//! ```rust
//! use casa_tables::{Table, TableSchema, ColumnSchema, TableKind};
//! use casa_types::*;
//!
//! let schema = TableSchema::new(vec![
//!     ColumnSchema::scalar("id", PrimitiveType::Int32),
...
//! };
//! use casa_types::{PrimitiveType, RecordValue, RecordField, Value, ScalarValue};
//!
//! // 1. Define the schema.
//! let schema = TableSchema::new(vec![
//!     ColumnSchema::scalar("id",   PrimitiveType::Int32),
...
pub use storage::{
    DEFAULT_TILE_IO_BYTES, DEFAULT_TILED_ARRAY_CACHE_BYTES, DataManagerInfo,
    STREAMING_TILED_COLUMN_BUFFER_BYTES, StorageError, StreamedTiledPrimitiveColumn,
